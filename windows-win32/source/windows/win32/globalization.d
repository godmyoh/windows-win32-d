...
int WideCharToMultiByte(uint, uint, const(wchar)*, int, PSTR, int, const(char)*, BOOL*);
...
int GetGeoInfoA(int, SYSGEOTYPE, PSTR, int, ushort);
int GetGeoInfoW(int, SYSGEOTYPE, PWSTR, int, ushort);
int GetGeoInfoEx(PWSTR, SYSGEOTYPE, PWSTR, int);
BOOL EnumSystemGeoID(uint, int, GEO_ENUMPROC);
BOOL EnumSystemGeoNames(uint, GEO_ENUMNAMEPROC, LPARAM);
int GetUserGeoID(SYSGEOCLASS);
