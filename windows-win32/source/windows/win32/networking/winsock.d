...
int WSCEnumProtocols32(int* lpiProtocols, WSAPROTOCOL_INFOW* lpProtocolBuffer, uint* lpdwBufferLength, int* lpErrno);
int WSCDeinstallProvider32(GUID* lpProviderId, int* lpErrno);
int WSCInstallProvider64_32(GUID* lpProviderId, const(wchar)* lpszProviderDllPath, const(WSAPROTOCOL_INFOW)* lpProtocolInfoList, uint dwNumberOfEntries, int* lpErrno);
int WSCGetProviderPath32(GUID* lpProviderId, PWSTR lpszProviderDllPath, int* lpProviderDllPathLen, int* lpErrno);
int WSCUpdateProvider32(GUID* lpProviderId, const(wchar)* lpszProviderDllPath, const(WSAPROTOCOL_INFOW)* lpProtocolInfoList, uint dwNumberOfEntries, int* lpErrno);
int WSCSetProviderInfo32(GUID* lpProviderId, WSC_PROVIDER_INFO_TYPE InfoType, ubyte* Info, ulong InfoSize, uint Flags, int* lpErrno);
int WSCGetProviderInfo32(GUID* lpProviderId, WSC_PROVIDER_INFO_TYPE InfoType, ubyte* Info, ulong* InfoSize, uint Flags, int* lpErrno);
int WSCEnumNameSpaceProviders32(uint* lpdwBufferLength, WSANAMESPACE_INFOW* lpnspBuffer);
int WSCEnumNameSpaceProvidersEx32(uint* lpdwBufferLength, WSANAMESPACE_INFOEXW* lpnspBuffer);
int WSCInstallNameSpace32(PWSTR lpszIdentifier, PWSTR lpszPathName, uint dwNameSpace, uint dwVersion, GUID* lpProviderId);
int WSCInstallNameSpaceEx32(PWSTR lpszIdentifier, PWSTR lpszPathName, uint dwNameSpace, uint dwVersion, GUID* lpProviderId, BLOB* lpProviderSpecific);
int WSCUnInstallNameSpace32(GUID* lpProviderId);
int WSCEnableNSProvider32(GUID* lpProviderId, BOOL fEnable);
int WSCInstallProviderAndChains64_32(GUID* lpProviderId, const(wchar)* lpszProviderDllPath, const(wchar)* lpszProviderDllPath32, const(wchar)* lpszLspName, uint dwServiceFlags, WSAPROTOCOL_INFOW* lpProtocolInfoList, uint dwNumberOfEntries, uint* lpdwCatalogEntryId, int* lpErrno);
...
int getsockopt(SOCKET s, int level, int optname, PSTR optval, int* optlen);
...
int recvfrom(SOCKET s, PSTR buf, int len, int flags, SOCKADDR* from, int* fromlen);
int select(int nfds, FD_SET* readfds, FD_SET* writefds, FD_SET* exceptfds, const(TIMEVAL)* timeout);
int send(SOCKET s, const(char)* buf, int len, SEND_RECV_FLAGS flags);
...
int gethostname(PSTR name, int namelen);
...
HANDLE WSAAsyncGetServByName(HWND hWnd, uint wMsg, const(char)* name, const(char)* proto, PSTR buf, int buflen);
HANDLE WSAAsyncGetServByPort(HWND hWnd, uint wMsg, int port, const(char)* proto, PSTR buf, int buflen);
HANDLE WSAAsyncGetProtoByName(HWND hWnd, uint wMsg, const(char)* name, PSTR buf, int buflen);
HANDLE WSAAsyncGetProtoByNumber(HWND hWnd, uint wMsg, int number, PSTR buf, int buflen);
HANDLE WSAAsyncGetHostByName(HWND hWnd, uint wMsg, const(char)* name, PSTR buf, int buflen);
HANDLE WSAAsyncGetHostByAddr(HWND hWnd, uint wMsg, const(char)* addr, int len, int type, PSTR buf, int buflen);
int WSACancelAsyncRequest(HANDLE hAsyncTaskHandle);
int WSAAsyncSelect(SOCKET s, HWND hWnd, uint wMsg, int lEvent);
SOCKET WSAAccept(SOCKET s, SOCKADDR* addr, int* addrlen, LPCONDITIONPROC lpfnCondition, ulong dwCallbackData);
BOOL WSACloseEvent(WSAEVENT hEvent);
int WSAConnect(SOCKET s, const(SOCKADDR)* name, int namelen, WSABUF* lpCallerData, WSABUF* lpCalleeData, QOS* lpSQOS, QOS* lpGQOS);
BOOL WSAConnectByNameW(SOCKET s, PWSTR nodename, PWSTR servicename, uint* LocalAddressLength, SOCKADDR* LocalAddress, uint* RemoteAddressLength, SOCKADDR* RemoteAddress, const(TIMEVAL)* timeout, OVERLAPPED* Reserved);
BOOL WSAConnectByNameA(SOCKET s, const(char)* nodename, const(char)* servicename, uint* LocalAddressLength, SOCKADDR* LocalAddress, uint* RemoteAddressLength, SOCKADDR* RemoteAddress, const(TIMEVAL)* timeout, OVERLAPPED* Reserved);
BOOL WSAConnectByList(SOCKET s, SOCKET_ADDRESS_LIST* SocketAddress, uint* LocalAddressLength, SOCKADDR* LocalAddress, uint* RemoteAddressLength, SOCKADDR* RemoteAddress, const(TIMEVAL)* timeout, OVERLAPPED* Reserved);
...
int WSAIoctl(SOCKET s, uint dwIoControlCode, void* lpvInBuffer, uint cbInBuffer, void* lpvOutBuffer, uint cbOutBuffer, uint* lpcbBytesReturned, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
SOCKET WSAJoinLeaf(SOCKET s, const(SOCKADDR)* name, int namelen, WSABUF* lpCallerData, WSABUF* lpCalleeData, QOS* lpSQOS, QOS* lpGQOS, uint dwFlags);
int WSANtohl(SOCKET s, uint netlong, uint* lphostlong);
int WSANtohs(SOCKET s, ushort netshort, ushort* lphostshort);
int WSARecv(SOCKET s, WSABUF* lpBuffers, uint dwBufferCount, uint* lpNumberOfBytesRecvd, uint* lpFlags, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
int WSARecvDisconnect(SOCKET s, WSABUF* lpInboundDisconnectData);
int WSARecvFrom(SOCKET s, WSABUF* lpBuffers, uint dwBufferCount, uint* lpNumberOfBytesRecvd, uint* lpFlags, SOCKADDR* lpFrom, int* lpFromlen, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
BOOL WSAResetEvent(WSAEVENT hEvent);
int WSASend(SOCKET s, WSABUF* lpBuffers, uint dwBufferCount, uint* lpNumberOfBytesSent, uint dwFlags, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
int WSASendMsg(SOCKET Handle, WSAMSG* lpMsg, uint dwFlags, uint* lpNumberOfBytesSent, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
int WSASendDisconnect(SOCKET s, WSABUF* lpOutboundDisconnectData);
int WSASendTo(SOCKET s, WSABUF* lpBuffers, uint dwBufferCount, uint* lpNumberOfBytesSent, uint dwFlags, const(SOCKADDR)* lpTo, int iTolen, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
...
WAIT_EVENT WSAWaitForMultipleEvents(uint cEvents, const(HANDLE)* lphEvents, BOOL fWaitAll, uint dwTimeout, BOOL fAlertable);
int WSAAddressToStringA(SOCKADDR* lpsaAddress, uint dwAddressLength, WSAPROTOCOL_INFOA* lpProtocolInfo, PSTR lpszAddressString, uint* lpdwAddressStringLength);
int WSAAddressToStringW(SOCKADDR* lpsaAddress, uint dwAddressLength, WSAPROTOCOL_INFOW* lpProtocolInfo, PWSTR lpszAddressString, uint* lpdwAddressStringLength);
int WSAStringToAddressA(PSTR AddressString, int AddressFamily, WSAPROTOCOL_INFOA* lpProtocolInfo, SOCKADDR* lpAddress, int* lpAddressLength);
int WSAStringToAddressW(PWSTR AddressString, int AddressFamily, WSAPROTOCOL_INFOW* lpProtocolInfo, SOCKADDR* lpAddress, int* lpAddressLength);
...
int WSANSPIoctl(HANDLE hLookup, uint dwControlCode, void* lpvInBuffer, uint cbInBuffer, void* lpvOutBuffer, uint cbOutBuffer, uint* lpcbBytesReturned, WSACOMPLETION* lpCompletion);
...
int WSAGetServiceClassInfoA(GUID* lpProviderId, GUID* lpServiceClassId, uint* lpdwBufSize, WSASERVICECLASSINFOA* lpServiceClassInfo);
int WSAGetServiceClassInfoW(GUID* lpProviderId, GUID* lpServiceClassId, uint* lpdwBufSize, WSASERVICECLASSINFOW* lpServiceClassInfo);
...
int WSAProviderConfigChange(HANDLE* lpNotificationHandle, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
int WSAPoll(WSAPOLLFD* fdArray, uint fds, int timeout);
uint ProcessSocketNotifications(HANDLE completionPort, uint registrationCount, SOCK_NOTIFY_REGISTRATION* registrationInfos, uint timeoutMs, uint completionCount, OVERLAPPED_ENTRY* completionPortEntries, uint* receivedEntryCount);
...
int RtlIpv4StringToAddressA(const(char)* S, BOOLEAN Strict, const(char)** Terminator, IN_ADDR* Addr);
int RtlIpv4StringToAddressExA(const(char)* AddressString, BOOLEAN Strict, IN_ADDR* Address, ushort* Port);
int RtlIpv4StringToAddressW(const(wchar)* S, BOOLEAN Strict, const(wchar)** Terminator, IN_ADDR* Addr);
int RtlIpv4StringToAddressExW(const(wchar)* AddressString, BOOLEAN Strict, IN_ADDR* Address, ushort* Port);
PSTR RtlIpv6AddressToStringA(const(IN6_ADDR)* Addr, PSTR S);
int RtlIpv6AddressToStringExA(const(IN6_ADDR)* Address, uint ScopeId, ushort Port, PSTR AddressString, uint* AddressStringLength);
PWSTR RtlIpv6AddressToStringW(const(IN6_ADDR)* Addr, PWSTR S);
int RtlIpv6AddressToStringExW(const(IN6_ADDR)* Address, uint ScopeId, ushort Port, PWSTR AddressString, uint* AddressStringLength);
int RtlIpv6StringToAddressA(const(char)* S, const(char)** Terminator, IN6_ADDR* Addr);
int RtlIpv6StringToAddressExA(const(char)* AddressString, IN6_ADDR* Address, uint* ScopeId, ushort* Port);
int RtlIpv6StringToAddressW(const(wchar)* S, const(wchar)** Terminator, IN6_ADDR* Addr);
...
int RtlEthernetStringToAddressA(const(char)* S, const(char)** Terminator, DL_EUI48* Addr);
int RtlEthernetStringToAddressW(const(wchar)* S, const(wchar)** Terminator, DL_EUI48* Addr);
int WSARecvEx(SOCKET s, PSTR buf, int len, int* flags);
BOOL TransmitFile(SOCKET hSocket, HANDLE hFile, uint nNumberOfBytesToWrite, uint nNumberOfBytesPerSend, OVERLAPPED* lpOverlapped, TRANSMIT_FILE_BUFFERS* lpTransmitBuffers, uint dwReserved);
BOOL AcceptEx(SOCKET sListenSocket, SOCKET sAcceptSocket, void* lpOutputBuffer, uint dwReceiveDataLength, uint dwLocalAddressLength, uint dwRemoteAddressLength, uint* lpdwBytesReceived, OVERLAPPED* lpOverlapped);
void GetAcceptExSockaddrs(void* lpOutputBuffer, uint dwReceiveDataLength, uint dwLocalAddressLength, uint dwRemoteAddressLength, SOCKADDR** LocalSockaddr, int* LocalSockaddrLength, SOCKADDR** RemoteSockaddr, int* RemoteSockaddrLength);
...
int WSCInstallProvider(GUID* lpProviderId, const(wchar)* lpszProviderDllPath, const(WSAPROTOCOL_INFOW)* lpProtocolInfoList, uint dwNumberOfEntries, int* lpErrno);
int WSCGetProviderPath(GUID* lpProviderId, PWSTR lpszProviderDllPath, int* lpProviderDllPathLen, int* lpErrno);
int WSCUpdateProvider(GUID* lpProviderId, const(wchar)* lpszProviderDllPath, const(WSAPROTOCOL_INFOW)* lpProtocolInfoList, uint dwNumberOfEntries, int* lpErrno);
int WSCSetProviderInfo(GUID* lpProviderId, WSC_PROVIDER_INFO_TYPE InfoType, ubyte* Info, ulong InfoSize, uint Flags, int* lpErrno);
int WSCGetProviderInfo(GUID* lpProviderId, WSC_PROVIDER_INFO_TYPE InfoType, ubyte* Info, ulong* InfoSize, uint Flags, int* lpErrno);
...
int WPUCompleteOverlappedRequest(SOCKET s, OVERLAPPED* lpOverlapped, uint dwError, uint cbTransferred, int* lpErrno);
int WSCInstallNameSpace(PWSTR lpszIdentifier, PWSTR lpszPathName, uint dwNameSpace, uint dwVersion, GUID* lpProviderId);
int WSCUnInstallNameSpace(GUID* lpProviderId);
int WSCInstallNameSpaceEx(PWSTR lpszIdentifier, PWSTR lpszPathName, uint dwNameSpace, uint dwVersion, GUID* lpProviderId, BLOB* lpProviderSpecific);
...
int GetAddressByNameA(uint dwNameSpace, GUID* lpServiceType, PSTR lpServiceName, int* lpiProtocols, uint dwResolution, SERVICE_ASYNC_INFO* lpServiceAsyncInfo, void* lpCsaddrBuffer, uint* lpdwBufferLength, PSTR lpAliasBuffer, uint* lpdwAliasBufferLength);
int GetAddressByNameW(uint dwNameSpace, GUID* lpServiceType, PWSTR lpServiceName, int* lpiProtocols, uint dwResolution, SERVICE_ASYNC_INFO* lpServiceAsyncInfo, void* lpCsaddrBuffer, uint* lpdwBufferLength, PWSTR lpAliasBuffer, uint* lpdwAliasBufferLength);
...
int SetServiceA(uint dwNameSpace, SET_SERVICE_OPERATION dwOperation, uint dwFlags, SERVICE_INFOA* lpServiceInfo, SERVICE_ASYNC_INFO* lpServiceAsyncInfo, uint* lpdwStatusFlags);
int SetServiceW(uint dwNameSpace, SET_SERVICE_OPERATION dwOperation, uint dwFlags, SERVICE_INFOW* lpServiceInfo, SERVICE_ASYNC_INFO* lpServiceAsyncInfo, uint* lpdwStatusFlags);
int GetServiceA(uint dwNameSpace, GUID* lpGuid, PSTR lpServiceName, uint dwProperties, void* lpBuffer, uint* lpdwBufferSize, SERVICE_ASYNC_INFO* lpServiceAsyncInfo);
int GetServiceW(uint dwNameSpace, GUID* lpGuid, PWSTR lpServiceName, uint dwProperties, void* lpBuffer, uint* lpdwBufferSize, SERVICE_ASYNC_INFO* lpServiceAsyncInfo);
...
int GetAddrInfoExA(const(char)* pName, const(char)* pServiceName, uint dwNameSpace, GUID* lpNspId, const(ADDRINFOEXA)* hints, ADDRINFOEXA** ppResult, TIMEVAL* timeout, OVERLAPPED* lpOverlapped, LPLOOKUPSERVICE_COMPLETION_ROUTINE lpCompletionRoutine, HANDLE* lpNameHandle);
int GetAddrInfoExW(const(wchar)* pName, const(wchar)* pServiceName, uint dwNameSpace, GUID* lpNspId, const(ADDRINFOEXW)* hints, ADDRINFOEXW** ppResult, TIMEVAL* timeout, OVERLAPPED* lpOverlapped, LPLOOKUPSERVICE_COMPLETION_ROUTINE lpCompletionRoutine, HANDLE* lpHandle);
int GetAddrInfoExCancel(HANDLE* lpHandle);
int GetAddrInfoExOverlappedResult(OVERLAPPED* lpOverlapped);
int SetAddrInfoExA(const(char)* pName, const(char)* pServiceName, SOCKET_ADDRESS* pAddresses, uint dwAddressCount, BLOB* lpBlob, uint dwFlags, uint dwNameSpace, GUID* lpNspId, TIMEVAL* timeout, OVERLAPPED* lpOverlapped, LPLOOKUPSERVICE_COMPLETION_ROUTINE lpCompletionRoutine, HANDLE* lpNameHandle);
int SetAddrInfoExW(const(wchar)* pName, const(wchar)* pServiceName, SOCKET_ADDRESS* pAddresses, uint dwAddressCount, BLOB* lpBlob, uint dwFlags, uint dwNameSpace, GUID* lpNspId, TIMEVAL* timeout, OVERLAPPED* lpOverlapped, LPLOOKUPSERVICE_COMPLETION_ROUTINE lpCompletionRoutine, HANDLE* lpNameHandle);
...
int getnameinfo(const(SOCKADDR)* pSockaddr, socklen_t SockaddrLength, PSTR pNodeBuffer, uint NodeBufferSize, PSTR pServiceBuffer, uint ServiceBufferSize, int Flags);
int GetNameInfoW(const(SOCKADDR)* pSockaddr, socklen_t SockaddrLength, PWSTR pNodeBuffer, uint NodeBufferSize, PWSTR pServiceBuffer, uint ServiceBufferSize, int Flags);
int inet_pton(int Family, const(char)* pszAddrString, void* pAddrBuf);
int InetPtonW(int Family, const(wchar)* pszAddrString, void* pAddrBuf);
PSTR inet_ntop(int Family, const(void)* pAddr, PSTR pStringBuf, ulong StringBufSize);
PWSTR InetNtopW(int Family, const(void)* pAddr, PWSTR pStringBuf, ulong StringBufSize);
int WSASetSocketSecurity(SOCKET Socket, const(SOCKET_SECURITY_SETTINGS)* SecuritySettings, uint SecuritySettingsLen, OVERLAPPED* Overlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE CompletionRoutine);
int WSAQuerySocketSecurity(SOCKET Socket, const(SOCKET_SECURITY_QUERY_TEMPLATE)* SecurityQueryTemplate, uint SecurityQueryTemplateLen, SOCKET_SECURITY_QUERY_INFO* SecurityQueryInfo, uint* SecurityQueryInfoLen, OVERLAPPED* Overlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE CompletionRoutine);
int WSASetSocketPeerTargetName(SOCKET Socket, const(SOCKET_PEER_TARGET_NAME)* PeerTargetName, uint PeerTargetNameLen, OVERLAPPED* Overlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE CompletionRoutine);
int WSADeleteSocketPeerTargetName(SOCKET Socket, const(SOCKADDR)* PeerAddr, uint PeerAddrLen, OVERLAPPED* Overlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE CompletionRoutine);
int WSAImpersonateSocketPeer(SOCKET Socket, const(SOCKADDR)* PeerAddr, uint PeerAddrLen);
...
enum TCP_INITIAL_RTO_UNSPECIFIED_MAX_SYN_RETRANSMISSIONS = 0xffff;
enum TCP_INITIAL_RTO_DEFAULT_RTT = 0x00000000;
enum TCP_INITIAL_RTO_DEFAULT_MAX_SYN_RETRANSMISSIONS = 0x00000000;
enum TCP_INITIAL_RTO_NO_SYN_RETRANSMISSIONS = 0xfffe;
...
enum IN4ADDR_ANY = 0x00000000;
enum IN4ADDR_LOOPBACK = 0x0100007f;
enum IN4ADDR_BROADCAST = 0xffffffff;
...
enum WSAID_WSASENDMSG = GUID(0xa441e712, 0x754f, 0x43ca, [0x84, 0xa7, 0xd, 0xee, 0x44, 0xcf, 0x60, 0x6d]);
...
enum WSA_INVALID_EVENT = 0x00000000;
...
enum SO_DONTLINGER = 0xffffffffffffff7f;
enum SO_EXCLUSIVEADDRUSE = 0xfffffffffffffffb;
...
alias LPCONDITIONPROC = int function(WSABUF* lpCallerId, WSABUF* lpCallerData, QOS* lpSQOS, QOS* lpGQOS, WSABUF* lpCalleeId, WSABUF* lpCalleeData, uint* g, ulong dwCallbackData);
alias LPWSAOVERLAPPED_COMPLETION_ROUTINE = void function(uint dwError, uint cbTransferred, OVERLAPPED* lpOverlapped, uint dwFlags);
...
alias LPFN_TRANSMITFILE = BOOL function(SOCKET hSocket, HANDLE hFile, uint nNumberOfBytesToWrite, uint nNumberOfBytesPerSend, OVERLAPPED* lpOverlapped, TRANSMIT_FILE_BUFFERS* lpTransmitBuffers, uint dwReserved);
alias LPFN_ACCEPTEX = BOOL function(SOCKET sListenSocket, SOCKET sAcceptSocket, void* lpOutputBuffer, uint dwReceiveDataLength, uint dwLocalAddressLength, uint dwRemoteAddressLength, uint* lpdwBytesReceived, OVERLAPPED* lpOverlapped);
alias LPFN_GETACCEPTEXSOCKADDRS = void function(void* lpOutputBuffer, uint dwReceiveDataLength, uint dwLocalAddressLength, uint dwRemoteAddressLength, SOCKADDR** LocalSockaddr, int* LocalSockaddrLength, SOCKADDR** RemoteSockaddr, int* RemoteSockaddrLength);
...
alias LPFN_TRANSMITPACKETS = BOOL function(SOCKET hSocket, TRANSMIT_PACKETS_ELEMENT* lpPacketArray, uint nElementCount, uint nSendSize, OVERLAPPED* lpOverlapped, uint dwFlags);
alias LPFN_CONNECTEX = BOOL function(SOCKET s, const(SOCKADDR)* name, int namelen, void* lpSendBuffer, uint dwSendDataLength, uint* lpdwBytesSent, OVERLAPPED* lpOverlapped);
...
alias LPFN_WSARECVMSG = int function(SOCKET s, WSAMSG* lpMsg, uint* lpdwNumberOfBytesRecvd, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
struct WSAPOLLDATA
{
    int result;
    uint fds;
...
alias LPFN_WSASENDMSG = int function(SOCKET s, WSAMSG* lpMsg, uint dwFlags, uint* lpNumberOfBytesSent, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
alias LPFN_WSAPOLL = int function(WSAPOLLFD* fdarray, uint nfds, int timeout);
alias LPFN_RIORECEIVE = BOOL function(RIO_RQ SocketQueue, RIO_BUF* pData, uint DataBufferCount, uint Flags, void* RequestContext);
alias LPFN_RIORECEIVEEX = int function(RIO_RQ SocketQueue, RIO_BUF* pData, uint DataBufferCount, RIO_BUF* pLocalAddress, RIO_BUF* pRemoteAddress, RIO_BUF* pControlContext, RIO_BUF* pFlags, uint Flags, void* RequestContext);
alias LPFN_RIOSEND = BOOL function(RIO_RQ SocketQueue, RIO_BUF* pData, uint DataBufferCount, uint Flags, void* RequestContext);
alias LPFN_RIOSENDEX = BOOL function(RIO_RQ SocketQueue, RIO_BUF* pData, uint DataBufferCount, RIO_BUF* pLocalAddress, RIO_BUF* pRemoteAddress, RIO_BUF* pControlContext, RIO_BUF* pFlags, uint Flags, void* RequestContext);
...
alias LPFN_RIOCREATECOMPLETIONQUEUE = RIO_CQ function(uint QueueSize, RIO_NOTIFICATION_COMPLETION* NotificationCompletion);
alias LPFN_RIOCREATEREQUESTQUEUE = RIO_RQ function(SOCKET Socket, uint MaxOutstandingReceive, uint MaxReceiveDataBuffers, uint MaxOutstandingSend, uint MaxSendDataBuffers, RIO_CQ ReceiveCQ, RIO_CQ SendCQ, void* SocketContext);
...
alias LPFN_RIOREGISTERBUFFER = RIO_BUFFERID function(PSTR DataBuffer, uint DataLength);
...
alias LPBLOCKINGCALLBACK = BOOL function(ulong dwContext);
alias LPWSAUSERAPC = void function(ulong dwContext);
alias LPWSPACCEPT = SOCKET function(SOCKET s, SOCKADDR* addr, int* addrlen, LPCONDITIONPROC lpfnCondition, ulong dwCallbackData, int* lpErrno);
alias LPWSPADDRESSTOSTRING = int function(SOCKADDR* lpsaAddress, uint dwAddressLength, WSAPROTOCOL_INFOW* lpProtocolInfo, PWSTR lpszAddressString, uint* lpdwAddressStringLength, int* lpErrno);
alias LPWSPASYNCSELECT = int function(SOCKET s, HWND hWnd, uint wMsg, int lEvent, int* lpErrno);
alias LPWSPBIND = int function(SOCKET s, const(SOCKADDR)* name, int namelen, int* lpErrno);
...
alias LPWSPCONNECT = int function(SOCKET s, const(SOCKADDR)* name, int namelen, WSABUF* lpCallerData, WSABUF* lpCalleeData, QOS* lpSQOS, QOS* lpGQOS, int* lpErrno);
alias LPWSPDUPLICATESOCKET = int function(SOCKET s, uint dwProcessId, WSAPROTOCOL_INFOW* lpProtocolInfo, int* lpErrno);
alias LPWSPENUMNETWORKEVENTS = int function(SOCKET s, HANDLE hEventObject, WSANETWORKEVENTS* lpNetworkEvents, int* lpErrno);
alias LPWSPEVENTSELECT = int function(SOCKET s, WSAEVENT hEventObject, int lNetworkEvents, int* lpErrno);
alias LPWSPGETOVERLAPPEDRESULT = BOOL function(SOCKET s, OVERLAPPED* lpOverlapped, uint* lpcbTransfer, BOOL fWait, uint* lpdwFlags, int* lpErrno);
alias LPWSPGETPEERNAME = int function(SOCKET s, SOCKADDR* name, int* namelen, int* lpErrno);
alias LPWSPGETSOCKNAME = int function(SOCKET s, SOCKADDR* name, int* namelen, int* lpErrno);
alias LPWSPGETSOCKOPT = int function(SOCKET s, int level, int optname, PSTR optval, int* optlen, int* lpErrno);
alias LPWSPGETQOSBYNAME = BOOL function(SOCKET s, WSABUF* lpQOSName, QOS* lpQOS, int* lpErrno);
alias LPWSPIOCTL = int function(SOCKET s, uint dwIoControlCode, void* lpvInBuffer, uint cbInBuffer, void* lpvOutBuffer, uint cbOutBuffer, uint* lpcbBytesReturned, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine, WSATHREADID* lpThreadId, int* lpErrno);
alias LPWSPJOINLEAF = SOCKET function(SOCKET s, const(SOCKADDR)* name, int namelen, WSABUF* lpCallerData, WSABUF* lpCalleeData, QOS* lpSQOS, QOS* lpGQOS, uint dwFlags, int* lpErrno);
alias LPWSPLISTEN = int function(SOCKET s, int backlog, int* lpErrno);
alias LPWSPRECV = int function(SOCKET s, WSABUF* lpBuffers, uint dwBufferCount, uint* lpNumberOfBytesRecvd, uint* lpFlags, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine, WSATHREADID* lpThreadId, int* lpErrno);
alias LPWSPRECVDISCONNECT = int function(SOCKET s, WSABUF* lpInboundDisconnectData, int* lpErrno);
alias LPWSPRECVFROM = int function(SOCKET s, WSABUF* lpBuffers, uint dwBufferCount, uint* lpNumberOfBytesRecvd, uint* lpFlags, SOCKADDR* lpFrom, int* lpFromlen, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine, WSATHREADID* lpThreadId, int* lpErrno);
alias LPWSPSELECT = int function(int nfds, FD_SET* readfds, FD_SET* writefds, FD_SET* exceptfds, const(TIMEVAL)* timeout, int* lpErrno);
alias LPWSPSEND = int function(SOCKET s, WSABUF* lpBuffers, uint dwBufferCount, uint* lpNumberOfBytesSent, uint dwFlags, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine, WSATHREADID* lpThreadId, int* lpErrno);
alias LPWSPSENDDISCONNECT = int function(SOCKET s, WSABUF* lpOutboundDisconnectData, int* lpErrno);
alias LPWSPSENDTO = int function(SOCKET s, WSABUF* lpBuffers, uint dwBufferCount, uint* lpNumberOfBytesSent, uint dwFlags, const(SOCKADDR)* lpTo, int iTolen, OVERLAPPED* lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine, WSATHREADID* lpThreadId, int* lpErrno);
alias LPWSPSETSOCKOPT = int function(SOCKET s, int level, int optname, const(char)* optval, int optlen, int* lpErrno);
alias LPWSPSHUTDOWN = int function(SOCKET s, int how, int* lpErrno);
alias LPWSPSOCKET = SOCKET function(int af, int type, int protocol, WSAPROTOCOL_INFOW* lpProtocolInfo, uint g, uint dwFlags, int* lpErrno);
alias LPWSPSTRINGTOADDRESS = int function(PWSTR AddressString, int AddressFamily, WSAPROTOCOL_INFOW* lpProtocolInfo, SOCKADDR* lpAddress, int* lpAddressLength, int* lpErrno);
...
alias LPWPUCREATESOCKETHANDLE = SOCKET function(uint dwCatalogEntryId, ulong dwContext, int* lpErrno);
alias LPWPUFDISSET = int function(SOCKET s, FD_SET* fdset);
alias LPWPUGETPROVIDERPATH = int function(GUID* lpProviderId, PWSTR lpszProviderDllPath, int* lpProviderDllPathLen, int* lpErrno);
...
alias LPWPUQUERYBLOCKINGCALLBACK = int function(uint dwCatalogEntryId, LPBLOCKINGCALLBACK* lplpfnCallback, ulong* lpdwContext, int* lpErrno);
alias LPWPUQUERYSOCKETHANDLECONTEXT = int function(SOCKET s, ulong* lpContext, int* lpErrno);
alias LPWPUQUEUEAPC = int function(WSATHREADID* lpThreadId, LPWSAUSERAPC lpfnUserApc, ulong dwContext, int* lpErrno);
...
alias LPWPUCOMPLETEOVERLAPPEDREQUEST = int function(SOCKET s, OVERLAPPED* lpOverlapped, uint dwError, uint cbTransferred, int* lpErrno);
...
alias LPWSPSTARTUP = int function(ushort wVersionRequested, WSPDATA* lpWSPData, WSAPROTOCOL_INFOW* lpProtocolInfo, WSPUPCALLTABLE UpcallTable, WSPPROC_TABLE* lpProcTable);
alias LPWSCENUMPROTOCOLS = int function(int* lpiProtocols, WSAPROTOCOL_INFOW* lpProtocolBuffer, uint* lpdwBufferLength, int* lpErrno);
alias LPWSCDEINSTALLPROVIDER = int function(GUID* lpProviderId, int* lpErrno);
alias LPWSCINSTALLPROVIDER = int function(GUID* lpProviderId, const(wchar)* lpszProviderDllPath, const(WSAPROTOCOL_INFOW)* lpProtocolInfoList, uint dwNumberOfEntries, int* lpErrno);
alias LPWSCGETPROVIDERPATH = int function(GUID* lpProviderId, PWSTR lpszProviderDllPath, int* lpProviderDllPathLen, int* lpErrno);
alias LPWSCUPDATEPROVIDER = int function(GUID* lpProviderId, const(wchar)* lpszProviderDllPath, const(WSAPROTOCOL_INFOW)* lpProtocolInfoList, uint dwNumberOfEntries, int* lpErrno);
...
alias LPWSCINSTALLNAMESPACE = int function(PWSTR lpszIdentifier, PWSTR lpszPathName, uint dwNameSpace, uint dwVersion, GUID* lpProviderId);
...
alias LPNSPLOOKUPSERVICEBEGIN = int function(GUID* lpProviderId, WSAQUERYSETW* lpqsRestrictions, WSASERVICECLASSINFOW* lpServiceClassInfo, uint dwControlFlags, HANDLE* lphLookup);
alias LPNSPLOOKUPSERVICENEXT = int function(HANDLE hLookup, uint dwControlFlags, uint* lpdwBufferLength, WSAQUERYSETW* lpqsResults);
alias LPNSPIOCTL = int function(HANDLE hLookup, uint dwControlCode, void* lpvInBuffer, uint cbInBuffer, void* lpvOutBuffer, uint cbOutBuffer, uint* lpcbBytesReturned, WSACOMPLETION* lpCompletion, WSATHREADID* lpThreadId);
alias LPNSPLOOKUPSERVICEEND = int function(HANDLE hLookup);
alias LPNSPSETSERVICE = int function(GUID* lpProviderId, WSASERVICECLASSINFOW* lpServiceClassInfo, WSAQUERYSETW* lpqsRegInfo, WSAESETSERVICEOP essOperation, uint dwControlFlags);
...
alias LPNSPGETSERVICECLASSINFO = int function(GUID* lpProviderId, uint* lpdwBufSize, WSASERVICECLASSINFOW* lpServiceClassInfo);
struct NSP_ROUTINE
{
    uint cbSize;
    uint dwMajorVersion;
...
alias LPNSPV2LOOKUPSERVICEBEGIN = int function(GUID* lpProviderId, WSAQUERYSET2W* lpqsRestrictions, uint dwControlFlags, void* lpvClientSessionArg, HANDLE* lphLookup);
alias LPNSPV2LOOKUPSERVICENEXTEX = void function(HANDLE hAsyncCall, HANDLE hLookup, uint dwControlFlags, uint* lpdwBufferLength, WSAQUERYSET2W* lpqsResults);
alias LPNSPV2LOOKUPSERVICEEND = int function(HANDLE hLookup);
alias LPNSPV2SETSERVICEEX = void function(HANDLE hAsyncCall, GUID* lpProviderId, WSAQUERYSET2W* lpqsRegInfo, WSAESETSERVICEOP essOperation, uint dwControlFlags, void* lpvClientSessionArg);
