...
uint HttpInitialize(HTTPAPI_VERSION Version, HTTP_INITIALIZE Flags, void* pReserved);
uint HttpTerminate(HTTP_INITIALIZE Flags, void* pReserved);
uint HttpCreateHttpHandle(HANDLE* RequestQueueHandle, uint Reserved);
uint HttpCreateRequestQueue(HTTPAPI_VERSION Version, const(wchar)* Name, SECURITY_ATTRIBUTES* SecurityAttributes, uint Flags, HANDLE* RequestQueueHandle);
uint HttpCloseRequestQueue(HANDLE RequestQueueHandle);
uint HttpSetRequestQueueProperty(HANDLE RequestQueueHandle, HTTP_SERVER_PROPERTY Property, void* PropertyInformation, uint PropertyInformationLength, uint Reserved1, void* Reserved2);
uint HttpQueryRequestQueueProperty(HANDLE RequestQueueHandle, HTTP_SERVER_PROPERTY Property, void* PropertyInformation, uint PropertyInformationLength, uint Reserved1, uint* ReturnLength, void* Reserved2);
uint HttpSetRequestProperty(HANDLE RequestQueueHandle, ulong Id, HTTP_REQUEST_PROPERTY PropertyId, void* Input, uint InputPropertySize, OVERLAPPED* Overlapped);
uint HttpShutdownRequestQueue(HANDLE RequestQueueHandle);
uint HttpReceiveClientCertificate(HANDLE RequestQueueHandle, ulong ConnectionId, uint Flags, HTTP_SSL_CLIENT_CERT_INFO* SslClientCertInfo, uint SslClientCertInfoSize, uint* BytesReceived, OVERLAPPED* Overlapped);
...
uint HttpQueryServerSessionProperty(ulong ServerSessionId, HTTP_SERVER_PROPERTY Property, void* PropertyInformation, uint PropertyInformationLength, uint* ReturnLength);
uint HttpSetServerSessionProperty(ulong ServerSessionId, HTTP_SERVER_PROPERTY Property, void* PropertyInformation, uint PropertyInformationLength);
...
uint HttpSetUrlGroupProperty(ulong UrlGroupId, HTTP_SERVER_PROPERTY Property, void* PropertyInformation, uint PropertyInformationLength);
uint HttpQueryUrlGroupProperty(ulong UrlGroupId, HTTP_SERVER_PROPERTY Property, void* PropertyInformation, uint PropertyInformationLength, uint* ReturnLength);
uint HttpPrepareUrl(void* Reserved, uint Flags, const(wchar)* Url, PWSTR* PreparedUrl);
uint HttpReceiveHttpRequest(HANDLE RequestQueueHandle, ulong RequestId, HTTP_RECEIVE_HTTP_REQUEST_FLAGS Flags, HTTP_REQUEST_V2* RequestBuffer, uint RequestBufferLength, uint* BytesReturned, OVERLAPPED* Overlapped);
uint HttpReceiveRequestEntityBody(HANDLE RequestQueueHandle, ulong RequestId, uint Flags, void* EntityBuffer, uint EntityBufferLength, uint* BytesReturned, OVERLAPPED* Overlapped);
...
uint HttpSendResponseEntityBody(HANDLE RequestQueueHandle, ulong RequestId, uint Flags, ushort EntityChunkCount, HTTP_DATA_CHUNK* EntityChunks, uint* BytesSent, void* Reserved1, uint Reserved2, OVERLAPPED* Overlapped, HTTP_LOG_DATA* LogData);
...
uint HttpDelegateRequestEx(HANDLE RequestQueueHandle, HANDLE DelegateQueueHandle, ulong RequestId, ulong DelegateUrlGroupId, uint PropertyInfoSetSize, HTTP_DELEGATE_REQUEST_PROPERTY_INFO* PropertyInfoSet);
uint HttpFindUrlGroupId(const(wchar)* FullyQualifiedUrl, HANDLE RequestQueueHandle, ulong* UrlGroupId);
...
uint HttpReadFragmentFromCache(HANDLE RequestQueueHandle, const(wchar)* UrlPrefix, HTTP_BYTE_RANGE* ByteRange, void* Buffer, uint BufferLength, uint* BytesRead, OVERLAPPED* Overlapped);
uint HttpSetServiceConfiguration(HANDLE ServiceHandle, HTTP_SERVICE_CONFIG_ID ConfigId, void* pConfigInformation, uint ConfigInformationLength, OVERLAPPED* pOverlapped);
uint HttpUpdateServiceConfiguration(HANDLE Handle, HTTP_SERVICE_CONFIG_ID ConfigId, void* ConfigInfo, uint ConfigInfoLength, OVERLAPPED* Overlapped);
uint HttpDeleteServiceConfiguration(HANDLE ServiceHandle, HTTP_SERVICE_CONFIG_ID ConfigId, void* pConfigInformation, uint ConfigInformationLength, OVERLAPPED* pOverlapped);
uint HttpQueryServiceConfiguration(HANDLE ServiceHandle, HTTP_SERVICE_CONFIG_ID ConfigId, void* pInput, uint InputLength, void* pOutput, uint OutputLength, uint* pReturnLength, OVERLAPPED* pOverlapped);
