...
interface IXmlReader : IUnknown
{
    ...
    HRESULT GetQualifiedName(const(wchar)**, uint*);
    HRESULT GetNamespaceUri(const(wchar)**, uint*);
    HRESULT GetLocalName(const(wchar)**, uint*);
    HRESULT GetPrefix(const(wchar)**, uint*);
    HRESULT GetValue(const(wchar)**, uint*);
    HRESULT ReadValueChunk(PWSTR, uint, uint*);
    HRESULT GetBaseUri(const(wchar)**, uint*);
    BOOL IsDefault();
    BOOL IsEmptyElement();
