...
import windows.win32.networkmanagement.ndis : IF_OPER_STATUS, NDIS_MEDIUM, NDIS_PHYSICAL_MEDIUM, NET_IF_ACCESS_TYPE, NET_IF_ADMIN_STATUS, NET_IF_COMPARTMENT_ID, NET_IF_CONNECTION_TYPE, NET_IF_DIRECTION_TYPE, NET_IF_MEDIA_CONNECT_STATE, NET_LUID_LH, TUNNEL_TYPE;
...
uint IcmpSendEcho(HANDLE IcmpHandle, uint DestinationAddress, void* RequestData, ushort RequestSize, IP_OPTION_INFORMATION* RequestOptions, void* ReplyBuffer, uint ReplySize, uint Timeout);
uint IcmpSendEcho2(HANDLE IcmpHandle, HANDLE Event, PIO_APC_ROUTINE ApcRoutine, void* ApcContext, uint DestinationAddress, void* RequestData, ushort RequestSize, IP_OPTION_INFORMATION* RequestOptions, void* ReplyBuffer, uint ReplySize, uint Timeout);
uint IcmpSendEcho2Ex(HANDLE IcmpHandle, HANDLE Event, PIO_APC_ROUTINE ApcRoutine, void* ApcContext, uint SourceAddress, uint DestinationAddress, void* RequestData, ushort RequestSize, IP_OPTION_INFORMATION* RequestOptions, void* ReplyBuffer, uint ReplySize, uint Timeout);
uint Icmp6SendEcho2(HANDLE IcmpHandle, HANDLE Event, PIO_APC_ROUTINE ApcRoutine, void* ApcContext, SOCKADDR_IN6* SourceAddress, SOCKADDR_IN6* DestinationAddress, void* RequestData, ushort RequestSize, IP_OPTION_INFORMATION* RequestOptions, void* ReplyBuffer, uint ReplySize, uint Timeout);
...
uint GetExtendedTcpTable(void* pTcpTable, uint* pdwSize, BOOL bOrder, uint ulAf, TCP_TABLE_CLASS TableClass, uint Reserved);
uint GetOwnerModuleFromTcpEntry(MIB_TCPROW_OWNER_MODULE* pTcpEntry, TCPIP_OWNER_MODULE_INFO_CLASS Class, void* pBuffer, uint* pdwSize);
uint GetUdpTable(MIB_UDPTABLE* UdpTable, uint* SizePointer, BOOL Order);
uint GetExtendedUdpTable(void* pUdpTable, uint* pdwSize, BOOL bOrder, uint ulAf, UDP_TABLE_CLASS TableClass, uint Reserved);
uint GetOwnerModuleFromUdpEntry(MIB_UDPROW_OWNER_MODULE* pUdpEntry, TCPIP_OWNER_MODULE_INFO_CLASS Class, void* pBuffer, uint* pdwSize);
...
uint GetPerTcpConnectionEStats(MIB_TCPROW_LH* Row, TCP_ESTATS_TYPE EstatsType, ubyte* Rw, uint RwVersion, uint RwSize, ubyte* Ros, uint RosVersion, uint RosSize, ubyte* Rod, uint RodVersion, uint RodSize);
uint SetPerTcpConnectionEStats(MIB_TCPROW_LH* Row, TCP_ESTATS_TYPE EstatsType, ubyte* Rw, uint RwVersion, uint RwSize, uint Offset);
uint GetPerTcp6ConnectionEStats(MIB_TCP6ROW* Row, TCP_ESTATS_TYPE EstatsType, ubyte* Rw, uint RwVersion, uint RwSize, ubyte* Ros, uint RosVersion, uint RosSize, ubyte* Rod, uint RodVersion, uint RodSize);
uint SetPerTcp6ConnectionEStats(MIB_TCP6ROW* Row, TCP_ESTATS_TYPE EstatsType, ubyte* Rw, uint RwVersion, uint RwSize, uint Offset);
uint GetOwnerModuleFromTcp6Entry(MIB_TCP6ROW_OWNER_MODULE* pTcpEntry, TCPIP_OWNER_MODULE_INFO_CLASS Class, void* pBuffer, uint* pdwSize);
uint GetUdp6Table(MIB_UDP6TABLE* Udp6Table, uint* SizePointer, BOOL Order);
uint GetOwnerModuleFromUdp6Entry(MIB_UDP6ROW_OWNER_MODULE* pUdpEntry, TCPIP_OWNER_MODULE_INFO_CLASS Class, void* pBuffer, uint* pdwSize);
uint GetOwnerModuleFromPidAndInfo(uint ulPid, ulong* pInfo, TCPIP_OWNER_MODULE_INFO_CLASS Class, void* pBuffer, uint* pdwSize);
...
WIN32_ERROR GetNetworkParams(FIXED_INFO_W2KSP1* pFixedInfo, uint* pOutBufLen);
uint GetAdaptersInfo(IP_ADAPTER_INFO* AdapterInfo, uint* SizePointer);
IP_ADAPTER_ORDER_MAP* GetAdapterOrderMap();
uint GetAdaptersAddresses(uint Family, GET_ADAPTERS_ADDRESSES_FLAGS Flags, void* Reserved, IP_ADAPTER_ADDRESSES_LH* AdapterAddresses, uint* SizePointer);
uint GetPerAdapterInfo(uint IfIndex, IP_PER_ADAPTER_INFO_W2KSP1* pPerAdapterInfo, uint* pOutBufLen);
uint GetInterfaceActiveTimestampCapabilities(const(NET_LUID_LH)* InterfaceLuid, INTERFACE_TIMESTAMP_CAPABILITIES* TimestampCapabilites);
uint GetInterfaceSupportedTimestampCapabilities(const(NET_LUID_LH)* InterfaceLuid, INTERFACE_TIMESTAMP_CAPABILITIES* TimestampCapabilites);
...
uint RegisterInterfaceTimestampConfigChange(PINTERFACE_TIMESTAMP_CONFIG_CHANGE_CALLBACK Callback, void* CallerContext, HIFTIMESTAMPCHANGE* NotificationHandle);
...
uint NotifyIfTimestampConfigChange(void* CallerContext, PINTERFACE_TIMESTAMP_CONFIG_CHANGE_CALLBACK Callback, HIFTIMESTAMPCHANGE* NotificationHandle);
...
uint ParseNetworkString(const(wchar)* NetworkString, uint Types, NET_ADDRESS_INFO* AddressInfo, ushort* PortNumber, ubyte* PrefixLength);
...
WIN32_ERROR NotifyIpInterfaceChange(ADDRESS_FAMILY Family, PIPINTERFACE_CHANGE_CALLBACK Callback, void* CallerContext, BOOLEAN InitialNotification, HANDLE* NotificationHandle);
WIN32_ERROR SetIpInterfaceEntry(MIB_IPINTERFACE_ROW* Row);
WIN32_ERROR GetIpNetworkConnectionBandwidthEstimates(uint InterfaceIndex, ADDRESS_FAMILY AddressFamily, MIB_IP_NETWORK_CONNECTION_BANDWIDTH_ESTIMATES* BandwidthEstimates);
...
WIN32_ERROR GetBestRoute2(NET_LUID_LH* InterfaceLuid, uint InterfaceIndex, const(SOCKADDR_INET)* SourceAddress, const(SOCKADDR_INET)* DestinationAddress, uint AddressSortOptions, MIB_IPFORWARD_ROW2* BestRoute, SOCKADDR_INET* BestSourceAddress);
...
WIN32_ERROR NotifyTeredoPortChange(PTEREDO_PORT_CHANGE_CALLBACK Callback, void* CallerContext, BOOLEAN InitialNotification, HANDLE* NotificationHandle);
...
WIN32_ERROR CreateSortedAddressPairs(const(SOCKADDR_IN6)* SourceAddressList, uint SourceAddressCount, const(SOCKADDR_IN6)* DestinationAddressList, uint DestinationAddressCount, uint AddressSortOptions, SOCKADDR_IN6_PAIR** SortedAddressPairList, uint* SortedAddressPairCount);
...
WIN32_ERROR ConvertInterfaceNameToLuidA(const(char)* InterfaceName, NET_LUID_LH* InterfaceLuid);
WIN32_ERROR ConvertInterfaceNameToLuidW(const(wchar)* InterfaceName, NET_LUID_LH* InterfaceLuid);
WIN32_ERROR ConvertInterfaceLuidToNameA(const(NET_LUID_LH)* InterfaceLuid, PSTR InterfaceName, ulong Length);
WIN32_ERROR ConvertInterfaceLuidToNameW(const(NET_LUID_LH)* InterfaceLuid, PWSTR InterfaceName, ulong Length);
...
WIN32_ERROR ConvertInterfaceLuidToAlias(const(NET_LUID_LH)* InterfaceLuid, PWSTR InterfaceAlias, ulong Length);
WIN32_ERROR ConvertInterfaceAliasToLuid(const(wchar)* InterfaceAlias, NET_LUID_LH* InterfaceLuid);
...
WIN32_ERROR GetNetworkInformation(const(GUID)* NetworkGuid, uint* CompartmentId, uint* SiteId, PWSTR NetworkName, uint Length);
WIN32_ERROR SetNetworkInformation(const(GUID)* NetworkGuid, NET_IF_COMPARTMENT_ID CompartmentId, const(wchar)* NetworkName);
...
WIN32_ERROR GetNetworkConnectivityHintForInterface(uint InterfaceIndex, NL_NETWORK_CONNECTIVITY_HINT* ConnectivityHint);
WIN32_ERROR NotifyNetworkConnectivityHintChange(PNETWORK_CONNECTIVITY_HINT_CHANGE_CALLBACK Callback, void* CallerContext, BOOLEAN InitialNotification, HANDLE* NotificationHandle);
uint PfCreateInterface(uint dwName, PFFORWARD_ACTION inAction, PFFORWARD_ACTION outAction, BOOL bUseLog, BOOL bMustBeUnique, void** ppInterface);
...
uint PfTestPacket(void* pInInterface, void* pOutInterface, uint cBytes, ubyte* pbPacket, PFFORWARD_ACTION* ppAction);
...
alias PIPINTERFACE_CHANGE_CALLBACK = void function(void* CallerContext, MIB_IPINTERFACE_ROW* Row, MIB_NOTIFICATION_TYPE NotificationType);
...
alias PUNICAST_IPADDRESS_CHANGE_CALLBACK = void function(void* CallerContext, MIB_UNICASTIPADDRESS_ROW* Row, MIB_NOTIFICATION_TYPE NotificationType);
alias PSTABLE_UNICAST_IPADDRESS_TABLE_CALLBACK = void function(void* CallerContext, MIB_UNICASTIPADDRESS_TABLE* AddressTable);
...
alias PIPFORWARD_CHANGE_CALLBACK = void function(void* CallerContext, MIB_IPFORWARD_ROW2* Row, MIB_NOTIFICATION_TYPE NotificationType);
...
alias PTEREDO_PORT_CHANGE_CALLBACK = void function(void* CallerContext, ushort Port, MIB_NOTIFICATION_TYPE NotificationType);
struct DNS_SETTINGS
{
    uint Version;
    ulong Flags;
...
alias PNETWORK_CONNECTIVITY_HINT_CHANGE_CALLBACK = void function(void* CallerContext, NL_NETWORK_CONNECTIVITY_HINT ConnectivityHint);
