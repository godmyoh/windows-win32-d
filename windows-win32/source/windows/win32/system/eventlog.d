...
BOOL BackupEventLogW(EventLogHandle, const(wchar)*);
BOOL CloseEventLog(EventLogHandle);
BOOL DeregisterEventSource(EventSourceHandle);
BOOL NotifyChangeEventLog(EventLogHandle, HANDLE);
BOOL GetNumberOfEventLogRecords(EventLogHandle, uint*);
BOOL GetOldestEventLogRecord(EventLogHandle, uint*);
EventLogHandle OpenEventLogA(const(char)*, const(char)*);
EventLogHandle OpenEventLogW(const(wchar)*, const(wchar)*);
EventSourceHandle RegisterEventSourceA(const(char)*, const(char)*);
EventSourceHandle RegisterEventSourceW(const(wchar)*, const(wchar)*);
EventLogHandle OpenBackupEventLogA(const(char)*, const(char)*);
