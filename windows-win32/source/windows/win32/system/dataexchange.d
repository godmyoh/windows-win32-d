...
LPARAM PackDDElParam(uint msg, ulong uiLo, ulong uiHi);
BOOL UnpackDDElParam(uint msg, LPARAM lParam, ulong* puiLo, ulong* puiHi);
BOOL FreeDDElParam(uint msg, LPARAM lParam);
LPARAM ReuseDDElParam(LPARAM lParam, uint msgIn, uint msgOut, ulong uiLo, ulong uiHi);
uint DdeInitializeA(uint* pidInst, PFNCALLBACK pfnCallback, DDE_INITIALIZE_COMMAND afCmd, uint ulRes);
uint DdeInitializeW(uint* pidInst, PFNCALLBACK pfnCallback, DDE_INITIALIZE_COMMAND afCmd, uint ulRes);
...
BOOL DdeSetUserHandle(HCONV hConv, uint id, ulong hUser);
...
HDDEDATA DdeClientTransaction(ubyte* pData, uint cbData, HCONV hConv, HSZ hszItem, uint wFmt, DDE_CLIENT_TRANSACTION_TYPE wType, uint dwTimeout, uint* pdwResult);
HDDEDATA DdeCreateDataHandle(uint idInst, ubyte* pSrc, uint cb, uint cbOff, HSZ hszItem, uint wFmt, uint afCmd);
HDDEDATA DdeAddData(HDDEDATA hData, ubyte* pSrc, uint cb, uint cbOff);
uint DdeGetData(HDDEDATA hData, ubyte* pDst, uint cbMax, uint cbOff);
...
HSZ DdeCreateStringHandleA(uint idInst, const(char)* psz, int iCodePage);
HSZ DdeCreateStringHandleW(uint idInst, const(wchar)* psz, int iCodePage);
...
HENHMETAFILE SetWinMetaFileBits(uint nSize, const(ubyte)* lpMeta16Data, HDC hdcRef, const(METAFILEPICT)* lpMFP);
...
alias PFNCALLBACK = HDDEDATA function(uint wType, uint wFmt, HCONV hConv, HSZ hsz1, HSZ hsz2, HDDEDATA hData, ulong dwData1, ulong dwData2);
