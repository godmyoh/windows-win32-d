...
HRESULT WHvGetCapability(WHV_CAPABILITY_CODE CapabilityCode, void* CapabilityBuffer, uint CapabilityBufferSizeInBytes, uint* WrittenSizeInBytes);
...
HRESULT WHvGetPartitionProperty(WHV_PARTITION_HANDLE Partition, WHV_PARTITION_PROPERTY_CODE PropertyCode, void* PropertyBuffer, uint PropertyBufferSizeInBytes, uint* WrittenSizeInBytes);
HRESULT WHvSetPartitionProperty(WHV_PARTITION_HANDLE Partition, WHV_PARTITION_PROPERTY_CODE PropertyCode, const(void)* PropertyBuffer, uint PropertyBufferSizeInBytes);
...
HRESULT WHvMapGpaRange(WHV_PARTITION_HANDLE Partition, void* SourceAddress, ulong GuestAddress, ulong SizeInBytes, WHV_MAP_GPA_RANGE_FLAGS Flags);
HRESULT WHvMapGpaRange2(WHV_PARTITION_HANDLE Partition, HANDLE Process, void* SourceAddress, ulong GuestAddress, ulong SizeInBytes, WHV_MAP_GPA_RANGE_FLAGS Flags);
HRESULT WHvUnmapGpaRange(WHV_PARTITION_HANDLE Partition, ulong GuestAddress, ulong SizeInBytes);
HRESULT WHvTranslateGva(WHV_PARTITION_HANDLE Partition, uint VpIndex, ulong Gva, WHV_TRANSLATE_GVA_FLAGS TranslateFlags, WHV_TRANSLATE_GVA_RESULT* TranslationResult, ulong* Gpa);
HRESULT WHvCreateVirtualProcessor(WHV_PARTITION_HANDLE Partition, uint VpIndex, uint Flags);
HRESULT WHvCreateVirtualProcessor2(WHV_PARTITION_HANDLE Partition, uint VpIndex, const(WHV_VIRTUAL_PROCESSOR_PROPERTY)* Properties, uint PropertyCount);
HRESULT WHvDeleteVirtualProcessor(WHV_PARTITION_HANDLE Partition, uint VpIndex);
HRESULT WHvRunVirtualProcessor(WHV_PARTITION_HANDLE Partition, uint VpIndex, void* ExitContext, uint ExitContextSizeInBytes);
HRESULT WHvCancelRunVirtualProcessor(WHV_PARTITION_HANDLE Partition, uint VpIndex, uint Flags);
HRESULT WHvGetVirtualProcessorRegisters(WHV_PARTITION_HANDLE Partition, uint VpIndex, const(WHV_REGISTER_NAME)* RegisterNames, uint RegisterCount, WHV_REGISTER_VALUE* RegisterValues);
HRESULT WHvSetVirtualProcessorRegisters(WHV_PARTITION_HANDLE Partition, uint VpIndex, const(WHV_REGISTER_NAME)* RegisterNames, uint RegisterCount, const(WHV_REGISTER_VALUE)* RegisterValues);
HRESULT WHvGetVirtualProcessorInterruptControllerState(WHV_PARTITION_HANDLE Partition, uint VpIndex, void* State, uint StateSize, uint* WrittenSize);
HRESULT WHvSetVirtualProcessorInterruptControllerState(WHV_PARTITION_HANDLE Partition, uint VpIndex, const(void)* State, uint StateSize);
HRESULT WHvRequestInterrupt(WHV_PARTITION_HANDLE Partition, const(WHV_INTERRUPT_CONTROL)* Interrupt, uint InterruptControlSize);
HRESULT WHvGetVirtualProcessorXsaveState(WHV_PARTITION_HANDLE Partition, uint VpIndex, void* Buffer, uint BufferSizeInBytes, uint* BytesWritten);
HRESULT WHvSetVirtualProcessorXsaveState(WHV_PARTITION_HANDLE Partition, uint VpIndex, const(void)* Buffer, uint BufferSizeInBytes);
HRESULT WHvQueryGpaRangeDirtyBitmap(WHV_PARTITION_HANDLE Partition, ulong GuestAddress, ulong RangeSizeInBytes, ulong* Bitmap, uint BitmapSizeInBytes);
HRESULT WHvGetPartitionCounters(WHV_PARTITION_HANDLE Partition, WHV_PARTITION_COUNTER_SET CounterSet, void* Buffer, uint BufferSizeInBytes, uint* BytesWritten);
HRESULT WHvGetVirtualProcessorCounters(WHV_PARTITION_HANDLE Partition, uint VpIndex, WHV_PROCESSOR_COUNTER_SET CounterSet, void* Buffer, uint BufferSizeInBytes, uint* BytesWritten);
HRESULT WHvGetVirtualProcessorInterruptControllerState2(WHV_PARTITION_HANDLE Partition, uint VpIndex, void* State, uint StateSize, uint* WrittenSize);
HRESULT WHvSetVirtualProcessorInterruptControllerState2(WHV_PARTITION_HANDLE Partition, uint VpIndex, const(void)* State, uint StateSize);
HRESULT WHvRegisterPartitionDoorbellEvent(WHV_PARTITION_HANDLE Partition, const(WHV_DOORBELL_MATCH_DATA)* MatchData, HANDLE EventHandle);
HRESULT WHvUnregisterPartitionDoorbellEvent(WHV_PARTITION_HANDLE Partition, const(WHV_DOORBELL_MATCH_DATA)* MatchData);
HRESULT WHvAdviseGpaRange(WHV_PARTITION_HANDLE Partition, const(WHV_MEMORY_RANGE_ENTRY)* GpaRanges, uint GpaRangesCount, WHV_ADVISE_GPA_RANGE_CODE Advice, const(void)* AdviceBuffer, uint AdviceBufferSizeInBytes);
HRESULT WHvReadGpaRange(WHV_PARTITION_HANDLE Partition, uint VpIndex, ulong GuestAddress, WHV_ACCESS_GPA_CONTROLS Controls, void* Data, uint DataSizeInBytes);
HRESULT WHvWriteGpaRange(WHV_PARTITION_HANDLE Partition, uint VpIndex, ulong GuestAddress, WHV_ACCESS_GPA_CONTROLS Controls, const(void)* Data, uint DataSizeInBytes);
HRESULT WHvSignalVirtualProcessorSynicEvent(WHV_PARTITION_HANDLE Partition, WHV_SYNIC_EVENT_PARAMETERS SynicEvent, BOOL* NewlySignaled);
HRESULT WHvGetVirtualProcessorState(WHV_PARTITION_HANDLE Partition, uint VpIndex, WHV_VIRTUAL_PROCESSOR_STATE_TYPE StateType, void* Buffer, uint BufferSizeInBytes, uint* BytesWritten);
HRESULT WHvSetVirtualProcessorState(WHV_PARTITION_HANDLE Partition, uint VpIndex, WHV_VIRTUAL_PROCESSOR_STATE_TYPE StateType, const(void)* Buffer, uint BufferSizeInBytes);
HRESULT WHvAllocateVpciResource(const(GUID)* ProviderId, WHV_ALLOCATE_VPCI_RESOURCE_FLAGS Flags, const(void)* ResourceDescriptor, uint ResourceDescriptorSizeInBytes, HANDLE* VpciResource);
HRESULT WHvCreateVpciDevice(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, HANDLE VpciResource, WHV_CREATE_VPCI_DEVICE_FLAGS Flags, HANDLE NotificationEventHandle);
HRESULT WHvDeleteVpciDevice(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId);
HRESULT WHvGetVpciDeviceProperty(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, WHV_VPCI_DEVICE_PROPERTY_CODE PropertyCode, void* PropertyBuffer, uint PropertyBufferSizeInBytes, uint* WrittenSizeInBytes);
HRESULT WHvGetVpciDeviceNotification(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, WHV_VPCI_DEVICE_NOTIFICATION* Notification, uint NotificationSizeInBytes);
HRESULT WHvMapVpciDeviceMmioRanges(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, uint* MappingCount, WHV_VPCI_MMIO_MAPPING** Mappings);
HRESULT WHvUnmapVpciDeviceMmioRanges(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId);
HRESULT WHvSetVpciDevicePowerState(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, DEVICE_POWER_STATE PowerState);
HRESULT WHvReadVpciDeviceRegister(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, const(WHV_VPCI_DEVICE_REGISTER)* Register, void* Data);
HRESULT WHvWriteVpciDeviceRegister(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, const(WHV_VPCI_DEVICE_REGISTER)* Register, const(void)* Data);
HRESULT WHvMapVpciDeviceInterrupt(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, uint Index, uint MessageCount, const(WHV_VPCI_INTERRUPT_TARGET)* Target, ulong* MsiAddress, uint* MsiData);
HRESULT WHvUnmapVpciDeviceInterrupt(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, uint Index);
HRESULT WHvRetargetVpciDeviceInterrupt(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, ulong MsiAddress, uint MsiData, const(WHV_VPCI_INTERRUPT_TARGET)* Target);
HRESULT WHvRequestVpciDeviceInterrupt(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, ulong MsiAddress, uint MsiData);
HRESULT WHvGetVpciDeviceInterruptTarget(WHV_PARTITION_HANDLE Partition, ulong LogicalDeviceId, uint Index, uint MultiMessageNumber, WHV_VPCI_INTERRUPT_TARGET* Target, uint TargetSizeInBytes, uint* BytesWritten);
HRESULT WHvCreateTrigger(WHV_PARTITION_HANDLE Partition, const(WHV_TRIGGER_PARAMETERS)* Parameters, void** TriggerHandle, HANDLE* EventHandle);
HRESULT WHvUpdateTriggerParameters(WHV_PARTITION_HANDLE Partition, const(WHV_TRIGGER_PARAMETERS)* Parameters, void* TriggerHandle);
HRESULT WHvDeleteTrigger(WHV_PARTITION_HANDLE Partition, void* TriggerHandle);
HRESULT WHvCreateNotificationPort(WHV_PARTITION_HANDLE Partition, const(WHV_NOTIFICATION_PORT_PARAMETERS)* Parameters, HANDLE EventHandle, void** PortHandle);
HRESULT WHvSetNotificationPortProperty(WHV_PARTITION_HANDLE Partition, void* PortHandle, WHV_NOTIFICATION_PORT_PROPERTY_CODE PropertyCode, ulong PropertyValue);
HRESULT WHvDeleteNotificationPort(WHV_PARTITION_HANDLE Partition, void* PortHandle);
HRESULT WHvPostVirtualProcessorSynicMessage(WHV_PARTITION_HANDLE Partition, uint VpIndex, uint SintIndex, const(void)* Message, uint MessageSizeInBytes);
HRESULT WHvGetVirtualProcessorCpuidOutput(WHV_PARTITION_HANDLE Partition, uint VpIndex, uint Eax, uint Ecx, WHV_CPUID_OUTPUT* CpuidOutput);
HRESULT WHvGetInterruptTargetVpSet(WHV_PARTITION_HANDLE Partition, ulong Destination, WHV_INTERRUPT_DESTINATION_MODE DestinationMode, uint* TargetVps, uint VpCount, uint* TargetVpCount);
...
HRESULT WHvEmulatorTryIoEmulation(void* Emulator, void* Context, const(WHV_VP_EXIT_CONTEXT)* VpContext, const(WHV_X64_IO_PORT_ACCESS_CONTEXT)* IoInstructionContext, WHV_EMULATOR_STATUS* EmulatorReturnStatus);
HRESULT WHvEmulatorTryMmioEmulation(void* Emulator, void* Context, const(WHV_VP_EXIT_CONTEXT)* VpContext, const(WHV_MEMORY_ACCESS_CONTEXT)* MmioInstructionContext, WHV_EMULATOR_STATUS* EmulatorReturnStatus);
...
HRESULT HdvCreateDeviceInstance(void* deviceHostHandle, HDV_DEVICE_TYPE deviceType, const(GUID)* deviceClassId, const(GUID)* deviceInstanceId, const(void)* deviceInterface, void* deviceContext, void** deviceHandle);
...
HRESULT HdvCreateGuestMemoryAperture(void* requestor, ulong guestPhysicalAddress, uint byteCount, BOOL writeProtected, void** mappedAddress);
...
HRESULT HdvRegisterDoorbell(void* requestor, HDV_PCI_BAR_SELECTOR BarIndex, ulong BarOffset, ulong TriggerValue, ulong Flags, HANDLE DoorbellEvent);
HRESULT HdvUnregisterDoorbell(void* requestor, HDV_PCI_BAR_SELECTOR BarIndex, ulong BarOffset, ulong TriggerValue, ulong Flags);
HRESULT HdvCreateSectionBackedMmioRange(void* requestor, HDV_PCI_BAR_SELECTOR barIndex, ulong offsetInPages, ulong lengthInPages, HDV_MMIO_MAPPING_FLAGS MappingFlags, HANDLE sectionHandle, ulong sectionOffsetInPages);
HRESULT HdvDestroySectionBackedMmioRange(void* requestor, HDV_PCI_BAR_SELECTOR barIndex, ulong offsetInPages);
HRESULT LocateSavedStateFiles(const(wchar)* vmName, const(wchar)* snapshotName, PWSTR* binPath, PWSTR* vsvPath, PWSTR* vmrsPath);
HRESULT LoadSavedStateFile(const(wchar)* vmrsFile, void** vmSavedStateDumpHandle);
HRESULT ApplyPendingSavedStateFileReplayLog(const(wchar)* vmrsFile);
HRESULT LoadSavedStateFiles(const(wchar)* binFile, const(wchar)* vsvFile, void** vmSavedStateDumpHandle);
...
HRESULT IsActiveVirtualTrustLevelEnabled(void* vmSavedStateDumpHandle, uint vpId, BOOL* activeVirtualTrustLevelEnabled);
...
HRESULT GetRegisterValue(void* vmSavedStateDumpHandle, uint vpId, uint registerId, VIRTUAL_PROCESSOR_REGISTER* registerValue);
...
HRESULT ReadGuestPhysicalAddress(void* vmSavedStateDumpHandle, ulong physicalAddress, void* buffer, uint bufferSize, uint* bytesRead);
HRESULT GuestVirtualAddressToPhysicalAddress(void* vmSavedStateDumpHandle, uint vpId, const(ulong) virtualAddress, ulong* physicalAddress, ulong* unmappedRegionSize);
HRESULT GetGuestPhysicalMemoryChunks(void* vmSavedStateDumpHandle, ulong* memoryChunkPageSize, GPA_MEMORY_CHUNK* memoryChunks, ulong* memoryChunkCount);
HRESULT GuestPhysicalAddressToRawSavedMemoryOffset(void* vmSavedStateDumpHandle, ulong physicalAddress, ulong* rawSavedMemoryOffset);
HRESULT ReadGuestRawSavedMemory(void* vmSavedStateDumpHandle, ulong rawSavedMemoryOffset, void* buffer, uint bufferSize, uint* bytesRead);
...
HRESULT ApplyGuestMemoryFix(void* vmSavedStateDumpHandle, uint vpId, ulong virtualAddress, void* fixBuffer, uint fixBufferSize);
HRESULT LoadSavedStateSymbolProvider(void* vmSavedStateDumpHandle, const(wchar)* userSymbols, BOOL force);
...
HRESULT SetSavedStateSymbolProviderDebugInfoCallback(void* vmSavedStateDumpHandle, GUEST_SYMBOLS_PROVIDER_DEBUG_INFO_CALLBACK Callback);
HRESULT LoadSavedStateModuleSymbols(void* vmSavedStateDumpHandle, const(char)* imageName, const(char)* moduleName, ulong baseAddress, uint sizeOfBase);
HRESULT LoadSavedStateModuleSymbolsEx(void* vmSavedStateDumpHandle, const(char)* imageName, uint imageTimestamp, const(char)* moduleName, ulong baseAddress, uint sizeOfBase);
HRESULT ResolveSavedStateGlobalVariableAddress(void* vmSavedStateDumpHandle, uint vpId, const(char)* globalName, ulong* virtualAddress, uint* size);
HRESULT ReadSavedStateGlobalVariable(void* vmSavedStateDumpHandle, uint vpId, const(char)* globalName, void* buffer, uint bufferSize);
HRESULT GetSavedStateSymbolTypeSize(void* vmSavedStateDumpHandle, uint vpId, const(char)* typeName, uint* size);
HRESULT FindSavedStateSymbolFieldInType(void* vmSavedStateDumpHandle, uint vpId, const(char)* typeName, const(wchar)* fieldName, uint* offset, BOOL* found);
HRESULT GetSavedStateSymbolFieldInfo(void* vmSavedStateDumpHandle, uint vpId, const(char)* typeName, PWSTR* typeFieldInfoMap);
HRESULT ScanMemoryForDosImages(void* vmSavedStateDumpHandle, uint vpId, ulong startAddress, ulong endAddress, void* callbackContext, FOUND_IMAGE_CALLBACK foundImageCallback, ulong* standaloneAddress, uint standaloneAddressCount);
HRESULT CallStackUnwind(void* vmSavedStateDumpHandle, uint vpId, MODULE_INFO* imageInfo, uint imageInfoCount, uint frameCount, PWSTR* callStack);
...
alias WHV_EMULATOR_GET_VIRTUAL_PROCESSOR_REGISTERS_CALLBACK = HRESULT function(void* Context, const(WHV_REGISTER_NAME)* RegisterNames, uint RegisterCount, WHV_REGISTER_VALUE* RegisterValues);
alias WHV_EMULATOR_SET_VIRTUAL_PROCESSOR_REGISTERS_CALLBACK = HRESULT function(void* Context, const(WHV_REGISTER_NAME)* RegisterNames, uint RegisterCount, const(WHV_REGISTER_VALUE)* RegisterValues);
alias WHV_EMULATOR_TRANSLATE_GVA_PAGE_CALLBACK = HRESULT function(void* Context, ulong Gva, WHV_TRANSLATE_GVA_FLAGS TranslateFlags, WHV_TRANSLATE_GVA_RESULT_CODE* TranslationResult, ulong* Gpa);
...
alias HDV_PCI_DEVICE_SET_CONFIGURATION = HRESULT function(void* deviceContext, uint configurationValueCount, const(wchar)** configurationValues);
alias HDV_PCI_DEVICE_GET_DETAILS = HRESULT function(void* deviceContext, HDV_PCI_PNP_ID* pnpId, uint probedBarsCount, uint* probedBars);
...
alias HDV_PCI_READ_INTERCEPTED_MEMORY = HRESULT function(void* deviceContext, HDV_PCI_BAR_SELECTOR barIndex, ulong offset, ulong length, ubyte* value);
alias HDV_PCI_WRITE_INTERCEPTED_MEMORY = HRESULT function(void* deviceContext, HDV_PCI_BAR_SELECTOR barIndex, ulong offset, ulong length, const(ubyte)* value);
...
alias GUEST_SYMBOLS_PROVIDER_DEBUG_INFO_CALLBACK = void function(const(char)* InfoMessage);
