...
BOOL GetSystemTimeAdjustmentPrecise(ulong* lpTimeAdjustment, ulong* lpTimeIncrement, BOOL* lpTimeAdjustmentDisabled);
...
BOOL GetLogicalProcessorInformationEx(LOGICAL_PROCESSOR_RELATIONSHIP RelationshipType, SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX* Buffer, uint* ReturnedLength);
void GetNativeSystemInfo(SYSTEM_INFO* lpSystemInfo);
void GetSystemTimePreciseAsFileTime(FILETIME* lpSystemTimeAsFileTime);
BOOL GetProductInfo(uint dwOSMajorVersion, uint dwOSMinorVersion, uint dwSpMajorVersion, uint dwSpMinorVersion, OS_PRODUCT_TYPE* pdwReturnedProductType);
ulong VerSetConditionMask(ulong ConditionMask, VER_FLAGS TypeMask, ubyte Condition);
BOOL GetOsSafeBootMode(uint* Flags);
uint EnumSystemFirmwareTables(FIRMWARE_TABLE_PROVIDER FirmwareTableProviderSignature, ubyte* pFirmwareTableEnumBuffer, uint BufferSize);
uint GetSystemFirmwareTable(FIRMWARE_TABLE_PROVIDER FirmwareTableProviderSignature, uint FirmwareTableID, ubyte* pFirmwareTableBuffer, uint BufferSize);
BOOL DnsHostnameToComputerNameExW(const(wchar)* Hostname, PWSTR ComputerName, uint* nSize);
...
BOOL GetProcessorSystemCycleTime(ushort Group, SYSTEM_PROCESSOR_CYCLE_TIME_INFORMATION* Buffer, uint* ReturnedLength);
...
BOOL GetSystemCpuSetInformation(SYSTEM_CPU_SET_INFORMATION* Information, uint BufferLength, uint* ReturnedLength, HANDLE Process, uint Flags);
...
BOOLEAN RtlGetProductInfo(uint OSMajorVersion, uint OSMinorVersion, uint SpMajorVersion, uint SpMinorVersion, uint* ReturnedProductType);
...
void RtlGetDeviceFamilyInfoEnum(ulong* pullUAPInfo, DEVICEFAMILYINFOENUM* pulDeviceFamily, DEVICEFAMILYDEVICEFORM* pulDeviceForm);
uint RtlConvertDeviceFamilyInfoToString(uint* pulDeviceFamilyBufferSize, uint* pulDeviceFormBufferSize, PWSTR DeviceFamily, PWSTR DeviceForm);
