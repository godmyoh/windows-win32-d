...
import windows.win32.foundation : BOOL, BSTR, CHAR, HANDLE, HRESULT, HWND, LPARAM, PSTR, PWSTR, SYSTEMTIME, VARIANT_BOOL;
import windows.win32.security : GENERIC_MAPPING, OBJECT_TYPE_LIST, PRIVILEGE_SET, PSECURITY_DESCRIPTOR, PSID;
...
uint GetAppliedGPOListA(uint dwFlags, const(char)* pMachineName, PSID pSidUser, GUID* pGuidExtension, GROUP_POLICY_OBJECTA** ppGPOList);
uint GetAppliedGPOListW(uint dwFlags, const(wchar)* pMachineName, PSID pSidUser, GUID* pGuidExtension, GROUP_POLICY_OBJECTW** ppGPOList);
uint ProcessGroupPolicyCompleted(GUID* extensionId, ulong pAsyncHandle, uint dwStatus);
uint ProcessGroupPolicyCompletedEx(GUID* extensionId, ulong pAsyncHandle, uint dwStatus, HRESULT RsopStatus);
HRESULT RsopAccessCheckByType(PSECURITY_DESCRIPTOR pSecurityDescriptor, PSID pPrincipalSelfSid, void* pRsopToken, uint dwDesiredAccessMask, OBJECT_TYPE_LIST* pObjectTypeList, uint ObjectTypeListLength, GENERIC_MAPPING* pGenericMapping, PRIVILEGE_SET* pPrivilegeSet, uint* pdwPrivilegeSetLength, uint* pdwGrantedAccessMask, BOOL* pbAccessStatus);
HRESULT RsopFileAccessCheck(PWSTR pszFileName, void* pRsopToken, uint dwDesiredAccessMask, uint* pdwGrantedAccessMask, BOOL* pbAccessStatus);
HRESULT RsopSetPolicySettingStatus(uint dwFlags, IWbemServices pServices, IWbemClassObject pSettingInstance, uint nInfo, POLICYSETTINGSTATUSINFO* pStatus);
...
enum REGISTRY_EXTENSION_GUID = GUID(0x35378eac, 0x683f, 0x11d2, [0xa8, 0x9a, 0x0, 0xc0, 0x4f, 0xbb, 0xcf, 0xa2]);
enum GROUP_POLICY_TRIGGER_EVENT_PROVIDER_GUID = GUID(0xbd2f4252, 0x5e1e, 0x49fc, [0x9a, 0x30, 0xf3, 0x97, 0x8a, 0xd8, 0x9e, 0xe2]);
enum MACHINE_POLICY_PRESENT_TRIGGER_GUID = GUID(0x659fcae6, 0x5bdb, 0x4da9, [0xb1, 0xff, 0xca, 0x2a, 0x17, 0x8d, 0x46, 0xe0]);
enum USER_POLICY_PRESENT_TRIGGER_GUID = GUID(0x54fb46c8, 0xf089, 0x464c, [0xb1, 0xfd, 0x59, 0xd1, 0xb6, 0x2c, 0x3b, 0x50]);
...
enum ADMXCOMMENTS_EXTENSION_GUID = GUID(0x6c5a2a86, 0x9eb3, 0x42b9, [0xaa, 0x83, 0xa7, 0x37, 0x1b, 0xa0, 0x11, 0xb9]);
...
interface IGPM : IDispatch
{
    ...
    HRESULT CreatePermission(BSTR bstrTrustee, GPMPermissionType perm, VARIANT_BOOL bInheritable, IGPMPermission* ppPerm);
...
interface IGPMBackup : IDispatch
{
    ...
    HRESULT Delete();
    HRESULT GenerateReport(GPMReportType gpmReportType, VARIANT* pvarGPMProgress, VARIANT* pvarGPMCancel, IGPMResult* ppIGPMResult);
...
interface IGPMConstants : IDispatch
{
    ...
    HRESULT get_SecurityFlags(VARIANT_BOOL vbOwner, VARIANT_BOOL vbGroup, VARIANT_BOOL vbDACL, VARIANT_BOOL vbSACL, int* pVal);
...
interface IGPMStarterGPO : IDispatch
{
    ...
    HRESULT Delete();
    HRESULT Save(BSTR bstrSaveFile, VARIANT_BOOL bOverwrite, VARIANT_BOOL bSaveAsSystem, VARIANT* bstrLanguage, VARIANT* bstrAuthor, VARIANT* bstrProduct, VARIANT* bstrUniqueID, VARIANT* bstrVersion, VARIANT* pvarGPMProgress, VARIANT* pvarGPMCancel, IGPMResult* ppIGPMResult);
    HRESULT Backup(BSTR bstrBackupDir, BSTR bstrComment, VARIANT* pvarGPMProgress, VARIANT* pvarGPMCancel, IGPMResult* ppIGPMResult);
...
interface IGPMDomain2 : IGPMDomain
{
    ...
    HRESULT LoadStarterGPO(BSTR bstrLoadFile, VARIANT_BOOL bOverwrite, VARIANT* pvarGPMProgress, VARIANT* pvarGPMCancel, IGPMResult* ppIGPMResult);
...
alias PFNPROCESSGROUPPOLICY = uint function(uint dwFlags, HANDLE hToken, HKEY hKeyRoot, GROUP_POLICY_OBJECTA* pDeletedGPOList, GROUP_POLICY_OBJECTA* pChangedGPOList, ulong pHandle, BOOL* pbAbort, PFNSTATUSMESSAGECALLBACK pStatusCallback);
alias PFNPROCESSGROUPPOLICYEX = uint function(uint dwFlags, HANDLE hToken, HKEY hKeyRoot, GROUP_POLICY_OBJECTA* pDeletedGPOList, GROUP_POLICY_OBJECTA* pChangedGPOList, ulong pHandle, BOOL* pbAbort, PFNSTATUSMESSAGECALLBACK pStatusCallback, IWbemServices pWbemServices, HRESULT* pRsopStatus);
struct RSOP_TARGET
{
...
alias PFNGENERATEGROUPPOLICY = uint function(uint dwFlags, BOOL* pbAbort, PWSTR pwszSite, RSOP_TARGET* pComputerTarget, RSOP_TARGET* pUserTarget);
...
interface IRSOPInformation : IUnknown
{
    ...
    HRESULT GetEventLogEntryText(PWSTR pszEventSource, PWSTR pszEventLogName, PWSTR pszEventTime, uint dwEventID, PWSTR* ppszText);
}
