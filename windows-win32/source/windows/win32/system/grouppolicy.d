...
HRESULT RsopAccessCheckByType(PSECURITY_DESCRIPTOR, PSID, void*, uint, OBJECT_TYPE_LIST*, uint, GENERIC_MAPPING*, PRIVILEGE_SET*, uint*, uint*, BOOL*);
HRESULT RsopFileAccessCheck(PWSTR, void*, uint, uint*, BOOL*);
...
interface IGroupPolicyObject : IUnknown
{
    ...
    HRESULT OpenDSGPO(PWSTR, GPO_OPEN_FLAGS);
    HRESULT OpenLocalMachineGPO(GPO_OPEN_FLAGS);
    HRESULT OpenRemoteMachineGPO(PWSTR, GPO_OPEN_FLAGS);
    ...
    HRESULT GetDSPath(uint, PWSTR, int);
    HRESULT GetFileSysPath(uint, PWSTR, int);
    HRESULT GetRegistryKey(GPO_SECTION, HKEY*);
    HRESULT GetOptions(GPO_OPTIONS*);
    HRESULT SetOptions(GPO_OPTIONS, uint);
    HRESULT GetType(GROUP_POLICY_OBJECT_TYPE*);
