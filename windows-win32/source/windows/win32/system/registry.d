...
WIN32_ERROR RegOpenUserClassesRoot(HANDLE hToken, uint dwOptions, uint samDesired, HKEY* phkResult);
...
WIN32_ERROR RegConnectRegistryA(const(char)* lpMachineName, HKEY hKey, HKEY* phkResult);
WIN32_ERROR RegConnectRegistryW(const(wchar)* lpMachineName, HKEY hKey, HKEY* phkResult);
int RegConnectRegistryExA(const(char)* lpMachineName, HKEY hKey, uint Flags, HKEY* phkResult);
int RegConnectRegistryExW(const(wchar)* lpMachineName, HKEY hKey, uint Flags, HKEY* phkResult);
WIN32_ERROR RegCreateKeyA(HKEY hKey, const(char)* lpSubKey, HKEY* phkResult);
WIN32_ERROR RegCreateKeyW(HKEY hKey, const(wchar)* lpSubKey, HKEY* phkResult);
WIN32_ERROR RegCreateKeyExA(HKEY hKey, const(char)* lpSubKey, uint Reserved, PSTR lpClass, REG_OPEN_CREATE_OPTIONS dwOptions, REG_SAM_FLAGS samDesired, const(SECURITY_ATTRIBUTES)* lpSecurityAttributes, HKEY* phkResult, REG_CREATE_KEY_DISPOSITION* lpdwDisposition);
WIN32_ERROR RegCreateKeyExW(HKEY hKey, const(wchar)* lpSubKey, uint Reserved, PWSTR lpClass, REG_OPEN_CREATE_OPTIONS dwOptions, REG_SAM_FLAGS samDesired, const(SECURITY_ATTRIBUTES)* lpSecurityAttributes, HKEY* phkResult, REG_CREATE_KEY_DISPOSITION* lpdwDisposition);
WIN32_ERROR RegCreateKeyTransactedA(HKEY hKey, const(char)* lpSubKey, uint Reserved, PSTR lpClass, REG_OPEN_CREATE_OPTIONS dwOptions, REG_SAM_FLAGS samDesired, const(SECURITY_ATTRIBUTES)* lpSecurityAttributes, HKEY* phkResult, REG_CREATE_KEY_DISPOSITION* lpdwDisposition, HANDLE hTransaction, void* pExtendedParemeter);
WIN32_ERROR RegCreateKeyTransactedW(HKEY hKey, const(wchar)* lpSubKey, uint Reserved, PWSTR lpClass, REG_OPEN_CREATE_OPTIONS dwOptions, REG_SAM_FLAGS samDesired, const(SECURITY_ATTRIBUTES)* lpSecurityAttributes, HKEY* phkResult, REG_CREATE_KEY_DISPOSITION* lpdwDisposition, HANDLE hTransaction, void* pExtendedParemeter);
WIN32_ERROR RegDeleteKeyA(HKEY hKey, const(char)* lpSubKey);
WIN32_ERROR RegDeleteKeyW(HKEY hKey, const(wchar)* lpSubKey);
WIN32_ERROR RegDeleteKeyExA(HKEY hKey, const(char)* lpSubKey, uint samDesired, uint Reserved);
WIN32_ERROR RegDeleteKeyExW(HKEY hKey, const(wchar)* lpSubKey, uint samDesired, uint Reserved);
WIN32_ERROR RegDeleteKeyTransactedA(HKEY hKey, const(char)* lpSubKey, uint samDesired, uint Reserved, HANDLE hTransaction, void* pExtendedParameter);
WIN32_ERROR RegDeleteKeyTransactedW(HKEY hKey, const(wchar)* lpSubKey, uint samDesired, uint Reserved, HANDLE hTransaction, void* pExtendedParameter);
...
WIN32_ERROR RegDeleteValueA(HKEY hKey, const(char)* lpValueName);
WIN32_ERROR RegDeleteValueW(HKEY hKey, const(wchar)* lpValueName);
...
WIN32_ERROR RegEnumKeyExA(HKEY hKey, uint dwIndex, PSTR lpName, uint* lpcchName, uint* lpReserved, PSTR lpClass, uint* lpcchClass, FILETIME* lpftLastWriteTime);
WIN32_ERROR RegEnumKeyExW(HKEY hKey, uint dwIndex, PWSTR lpName, uint* lpcchName, uint* lpReserved, PWSTR lpClass, uint* lpcchClass, FILETIME* lpftLastWriteTime);
WIN32_ERROR RegEnumValueA(HKEY hKey, uint dwIndex, PSTR lpValueName, uint* lpcchValueName, uint* lpReserved, uint* lpType, ubyte* lpData, uint* lpcbData);
WIN32_ERROR RegEnumValueW(HKEY hKey, uint dwIndex, PWSTR lpValueName, uint* lpcchValueName, uint* lpReserved, uint* lpType, ubyte* lpData, uint* lpcbData);
WIN32_ERROR RegFlushKey(HKEY hKey);
WIN32_ERROR RegGetKeySecurity(HKEY hKey, OBJECT_SECURITY_INFORMATION SecurityInformation, PSECURITY_DESCRIPTOR pSecurityDescriptor, uint* lpcbSecurityDescriptor);
WIN32_ERROR RegLoadKeyA(HKEY hKey, const(char)* lpSubKey, const(char)* lpFile);
WIN32_ERROR RegLoadKeyW(HKEY hKey, const(wchar)* lpSubKey, const(wchar)* lpFile);
...
WIN32_ERROR RegOpenKeyA(HKEY hKey, const(char)* lpSubKey, HKEY* phkResult);
WIN32_ERROR RegOpenKeyW(HKEY hKey, const(wchar)* lpSubKey, HKEY* phkResult);
WIN32_ERROR RegOpenKeyExA(HKEY hKey, const(char)* lpSubKey, uint ulOptions, REG_SAM_FLAGS samDesired, HKEY* phkResult);
WIN32_ERROR RegOpenKeyExW(HKEY hKey, const(wchar)* lpSubKey, uint ulOptions, REG_SAM_FLAGS samDesired, HKEY* phkResult);
WIN32_ERROR RegOpenKeyTransactedA(HKEY hKey, const(char)* lpSubKey, uint ulOptions, REG_SAM_FLAGS samDesired, HKEY* phkResult, HANDLE hTransaction, void* pExtendedParemeter);
WIN32_ERROR RegOpenKeyTransactedW(HKEY hKey, const(wchar)* lpSubKey, uint ulOptions, REG_SAM_FLAGS samDesired, HKEY* phkResult, HANDLE hTransaction, void* pExtendedParemeter);
WIN32_ERROR RegQueryInfoKeyA(HKEY hKey, PSTR lpClass, uint* lpcchClass, uint* lpReserved, uint* lpcSubKeys, uint* lpcbMaxSubKeyLen, uint* lpcbMaxClassLen, uint* lpcValues, uint* lpcbMaxValueNameLen, uint* lpcbMaxValueLen, uint* lpcbSecurityDescriptor, FILETIME* lpftLastWriteTime);
WIN32_ERROR RegQueryInfoKeyW(HKEY hKey, PWSTR lpClass, uint* lpcchClass, uint* lpReserved, uint* lpcSubKeys, uint* lpcbMaxSubKeyLen, uint* lpcbMaxClassLen, uint* lpcValues, uint* lpcbMaxValueNameLen, uint* lpcbMaxValueLen, uint* lpcbSecurityDescriptor, FILETIME* lpftLastWriteTime);
WIN32_ERROR RegQueryValueA(HKEY hKey, const(char)* lpSubKey, PSTR lpData, int* lpcbData);
WIN32_ERROR RegQueryValueW(HKEY hKey, const(wchar)* lpSubKey, PWSTR lpData, int* lpcbData);
WIN32_ERROR RegQueryMultipleValuesA(HKEY hKey, VALENTA* val_list, uint num_vals, PSTR lpValueBuf, uint* ldwTotsize);
WIN32_ERROR RegQueryMultipleValuesW(HKEY hKey, VALENTW* val_list, uint num_vals, PWSTR lpValueBuf, uint* ldwTotsize);
WIN32_ERROR RegQueryValueExA(HKEY hKey, const(char)* lpValueName, uint* lpReserved, REG_VALUE_TYPE* lpType, ubyte* lpData, uint* lpcbData);
WIN32_ERROR RegQueryValueExW(HKEY hKey, const(wchar)* lpValueName, uint* lpReserved, REG_VALUE_TYPE* lpType, ubyte* lpData, uint* lpcbData);
WIN32_ERROR RegReplaceKeyA(HKEY hKey, const(char)* lpSubKey, const(char)* lpNewFile, const(char)* lpOldFile);
WIN32_ERROR RegReplaceKeyW(HKEY hKey, const(wchar)* lpSubKey, const(wchar)* lpNewFile, const(wchar)* lpOldFile);
WIN32_ERROR RegRestoreKeyA(HKEY hKey, const(char)* lpFile, uint dwFlags);
WIN32_ERROR RegRestoreKeyW(HKEY hKey, const(wchar)* lpFile, uint dwFlags);
WIN32_ERROR RegRenameKey(HKEY hKey, const(wchar)* lpSubKeyName, const(wchar)* lpNewKeyName);
WIN32_ERROR RegSaveKeyA(HKEY hKey, const(char)* lpFile, const(SECURITY_ATTRIBUTES)* lpSecurityAttributes);
WIN32_ERROR RegSaveKeyW(HKEY hKey, const(wchar)* lpFile, const(SECURITY_ATTRIBUTES)* lpSecurityAttributes);
...
WIN32_ERROR RegSetValueA(HKEY hKey, const(char)* lpSubKey, REG_VALUE_TYPE dwType, const(char)* lpData, uint cbData);
WIN32_ERROR RegSetValueW(HKEY hKey, const(wchar)* lpSubKey, REG_VALUE_TYPE dwType, const(wchar)* lpData, uint cbData);
WIN32_ERROR RegSetValueExA(HKEY hKey, const(char)* lpValueName, uint Reserved, REG_VALUE_TYPE dwType, const(ubyte)* lpData, uint cbData);
WIN32_ERROR RegSetValueExW(HKEY hKey, const(wchar)* lpValueName, uint Reserved, REG_VALUE_TYPE dwType, const(ubyte)* lpData, uint cbData);
WIN32_ERROR RegUnLoadKeyA(HKEY hKey, const(char)* lpSubKey);
WIN32_ERROR RegUnLoadKeyW(HKEY hKey, const(wchar)* lpSubKey);
WIN32_ERROR RegDeleteKeyValueA(HKEY hKey, const(char)* lpSubKey, const(char)* lpValueName);
WIN32_ERROR RegDeleteKeyValueW(HKEY hKey, const(wchar)* lpSubKey, const(wchar)* lpValueName);
WIN32_ERROR RegSetKeyValueA(HKEY hKey, const(char)* lpSubKey, const(char)* lpValueName, uint dwType, const(void)* lpData, uint cbData);
WIN32_ERROR RegSetKeyValueW(HKEY hKey, const(wchar)* lpSubKey, const(wchar)* lpValueName, uint dwType, const(void)* lpData, uint cbData);
WIN32_ERROR RegDeleteTreeA(HKEY hKey, const(char)* lpSubKey);
WIN32_ERROR RegDeleteTreeW(HKEY hKey, const(wchar)* lpSubKey);
WIN32_ERROR RegCopyTreeA(HKEY hKeySrc, const(char)* lpSubKey, HKEY hKeyDest);
WIN32_ERROR RegGetValueA(HKEY hkey, const(char)* lpSubKey, const(char)* lpValue, REG_ROUTINE_FLAGS dwFlags, REG_VALUE_TYPE* pdwType, void* pvData, uint* pcbData);
WIN32_ERROR RegGetValueW(HKEY hkey, const(wchar)* lpSubKey, const(wchar)* lpValue, REG_ROUTINE_FLAGS dwFlags, REG_VALUE_TYPE* pdwType, void* pvData, uint* pcbData);
WIN32_ERROR RegCopyTreeW(HKEY hKeySrc, const(wchar)* lpSubKey, HKEY hKeyDest);
WIN32_ERROR RegLoadMUIStringA(HKEY hKey, const(char)* pszValue, PSTR pszOutBuf, uint cbOutBuf, uint* pcbData, uint Flags, const(char)* pszDirectory);
WIN32_ERROR RegLoadMUIStringW(HKEY hKey, const(wchar)* pszValue, PWSTR pszOutBuf, uint cbOutBuf, uint* pcbData, uint Flags, const(wchar)* pszDirectory);
WIN32_ERROR RegLoadAppKeyA(const(char)* lpFile, HKEY* phkResult, uint samDesired, uint dwOptions, uint Reserved);
WIN32_ERROR RegLoadAppKeyW(const(wchar)* lpFile, HKEY* phkResult, uint samDesired, uint dwOptions, uint Reserved);
WIN32_ERROR RegSaveKeyExA(HKEY hKey, const(char)* lpFile, const(SECURITY_ATTRIBUTES)* lpSecurityAttributes, REG_SAVE_FORMAT Flags);
WIN32_ERROR RegSaveKeyExW(HKEY hKey, const(wchar)* lpFile, const(SECURITY_ATTRIBUTES)* lpSecurityAttributes, REG_SAVE_FORMAT Flags);
WIN32_ERROR GetRegistryValueWithFallbackW(HKEY hkeyPrimary, const(wchar)* pwszPrimarySubKey, HKEY hkeyFallback, const(wchar)* pwszFallbackSubKey, const(wchar)* pwszValue, uint dwFlags, uint* pdwType, void* pvData, uint cbDataIn, uint* pcbDataOut);
...
alias PQUERYHANDLER = uint function(void* keycontext, val_context* val_list, uint num_vals, void* outputbuffer, uint* total_outlen, uint input_blen);
struct REG_PROVIDER
{
