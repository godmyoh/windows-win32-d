...
HRESULT RegisterDeviceWithManagementUsingAADDeviceCredentials2(const(wchar)* MDMApplicationID);
...
HRESULT GetDeviceManagementConfigInfo(const(wchar)* providerID, uint* configStringBufferLength, PWSTR configString);
HRESULT SetDeviceManagementConfigInfo(const(wchar)* providerID, const(wchar)* configString);
...
HRESULT ApplyLocalManagementSyncML(const(wchar)* syncMLRequest, PWSTR* syncMLResult);
...
enum MENROLL_E_CUSTOMSERVERERROR = 0xffffffff80180032;
