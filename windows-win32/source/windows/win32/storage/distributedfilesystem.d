...
uint NetDfsGetSecurity(PWSTR DfsEntryPath, uint SecurityInformation, PSECURITY_DESCRIPTOR* ppSecurityDescriptor, uint* lpcbSecurityDescriptor);
uint NetDfsSetSecurity(PWSTR DfsEntryPath, uint SecurityInformation, PSECURITY_DESCRIPTOR pSecurityDescriptor);
uint NetDfsGetStdContainerSecurity(PWSTR MachineName, uint SecurityInformation, PSECURITY_DESCRIPTOR* ppSecurityDescriptor, uint* lpcbSecurityDescriptor);
uint NetDfsSetStdContainerSecurity(PWSTR MachineName, uint SecurityInformation, PSECURITY_DESCRIPTOR pSecurityDescriptor);
uint NetDfsGetFtContainerSecurity(PWSTR DomainName, uint SecurityInformation, PSECURITY_DESCRIPTOR* ppSecurityDescriptor, uint* lpcbSecurityDescriptor);
uint NetDfsSetFtContainerSecurity(PWSTR DomainName, uint SecurityInformation, PSECURITY_DESCRIPTOR pSecurityDescriptor);
uint NetDfsGetSupportedNamespaceVersion(DFS_NAMESPACE_VERSION_ORIGIN Origin, PWSTR pName, DFS_SUPPORTED_NAMESPACE_VERSION_INFO** ppVersionInfo);
