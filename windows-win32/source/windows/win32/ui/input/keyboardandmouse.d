...
HKL LoadKeyboardLayoutA(const(char)* pwszKLID, ACTIVATE_KEYBOARD_LAYOUT_FLAGS Flags);
HKL LoadKeyboardLayoutW(const(wchar)* pwszKLID, ACTIVATE_KEYBOARD_LAYOUT_FLAGS Flags);
HKL ActivateKeyboardLayout(HKL hkl, ACTIVATE_KEYBOARD_LAYOUT_FLAGS Flags);
int ToUnicodeEx(uint wVirtKey, uint wScanCode, const(ubyte)* lpKeyState, PWSTR pwszBuff, int cchBuff, uint wFlags, HKL dwhkl);
...
int GetMouseMovePointsEx(uint cbSize, MOUSEMOVEPOINT* lppt, MOUSEMOVEPOINT* lpptBuf, int nBufPoints, GET_MOUSE_MOVE_POINTS_EX_RESOLUTION resolution);
...
void keybd_event(ubyte bVk, ubyte bScan, KEYBD_EVENT_FLAGS dwFlags, ulong dwExtraInfo);
void mouse_event(MOUSE_EVENT_FLAGS dwFlags, int dx, int dy, int dwData, ulong dwExtraInfo);
