...
long GetWindowLongPtrA(HWND hWnd, WINDOW_LONG_PTR_INDEX nIndex);
long GetWindowLongPtrW(HWND hWnd, WINDOW_LONG_PTR_INDEX nIndex);
long SetWindowLongPtrA(HWND hWnd, WINDOW_LONG_PTR_INDEX nIndex, long dwNewLong);
long SetWindowLongPtrW(HWND hWnd, WINDOW_LONG_PTR_INDEX nIndex, long dwNewLong);
ulong GetClassLongPtrA(HWND hWnd, GET_CLASS_LONG_INDEX nIndex);
ulong GetClassLongPtrW(HWND hWnd, GET_CLASS_LONG_INDEX nIndex);
ulong SetClassLongPtrA(HWND hWnd, GET_CLASS_LONG_INDEX nIndex, long dwNewLong);
ulong SetClassLongPtrW(HWND hWnd, GET_CLASS_LONG_INDEX nIndex, long dwNewLong);
int wvsprintfA(PSTR param0, const(char)* param1, byte* arglist);
int wvsprintfW(PWSTR param0, const(wchar)* param1, byte* arglist);
int wsprintfA(PSTR param0, const(char)* param1);
int wsprintfW(PWSTR param0, const(wchar)* param1);
...
BOOL PeekMessageA(MSG* lpMsg, HWND hWnd, uint wMsgFilterMin, uint wMsgFilterMax, PEEK_MESSAGE_REMOVE_TYPE wRemoveMsg);
BOOL PeekMessageW(MSG* lpMsg, HWND hWnd, uint wMsgFilterMin, uint wMsgFilterMax, PEEK_MESSAGE_REMOVE_TYPE wRemoveMsg);
uint GetMessagePos();
int GetMessageTime();
...
LRESULT SendMessageTimeoutA(HWND hWnd, uint Msg, WPARAM wParam, LPARAM lParam, SEND_MESSAGE_TIMEOUT_FLAGS fuFlags, uint uTimeout, ulong* lpdwResult);
LRESULT SendMessageTimeoutW(HWND hWnd, uint Msg, WPARAM wParam, LPARAM lParam, SEND_MESSAGE_TIMEOUT_FLAGS fuFlags, uint uTimeout, ulong* lpdwResult);
...
BOOL SendMessageCallbackA(HWND hWnd, uint Msg, WPARAM wParam, LPARAM lParam, SENDASYNCPROC lpResultCallBack, ulong dwData);
BOOL SendMessageCallbackW(HWND hWnd, uint Msg, WPARAM wParam, LPARAM lParam, SENDASYNCPROC lpResultCallBack, ulong dwData);
...
HWND CreateWindowExA(WINDOW_EX_STYLE dwExStyle, const(char)* lpClassName, const(char)* lpWindowName, WINDOW_STYLE dwStyle, int X, int Y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, void* lpParam);
HWND CreateWindowExW(WINDOW_EX_STYLE dwExStyle, const(wchar)* lpClassName, const(wchar)* lpWindowName, WINDOW_STYLE dwStyle, int X, int Y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, void* lpParam);
...
BOOL GetLayeredWindowAttributes(HWND hwnd, COLORREF* pcrKey, ubyte* pbAlpha, LAYERED_WINDOW_ATTRIBUTES_FLAGS* pdwFlags);
...
long DialogBoxParamA(HINSTANCE hInstance, const(char)* lpTemplateName, HWND hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam);
long DialogBoxParamW(HINSTANCE hInstance, const(wchar)* lpTemplateName, HWND hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam);
long DialogBoxIndirectParamA(HINSTANCE hInstance, DLGTEMPLATE* hDialogTemplate, HWND hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam);
long DialogBoxIndirectParamW(HINSTANCE hInstance, DLGTEMPLATE* hDialogTemplate, HWND hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam);
BOOL EndDialog(HWND hDlg, long nResult);
...
BOOL CharToOemA(const(char)* pSrc, PSTR pDst);
BOOL CharToOemW(const(wchar)* pSrc, PSTR pDst);
BOOL OemToCharA(const(char)* pSrc, PSTR pDst);
BOOL OemToCharW(const(char)* pSrc, PWSTR pDst);
BOOL CharToOemBuffA(const(char)* lpszSrc, PSTR lpszDst, uint cchDstLength);
BOOL CharToOemBuffW(const(wchar)* lpszSrc, PSTR lpszDst, uint cchDstLength);
BOOL OemToCharBuffA(const(char)* lpszSrc, PSTR lpszDst, uint cchDstLength);
BOOL OemToCharBuffW(const(char)* lpszSrc, PWSTR lpszDst, uint cchDstLength);
...
PSTR CharNextA(const(char)* lpsz);
PWSTR CharNextW(const(wchar)* lpsz);
PSTR CharPrevA(const(char)* lpszStart, const(char)* lpszCurrent);
PWSTR CharPrevW(const(wchar)* lpszStart, const(wchar)* lpszCurrent);
PSTR CharNextExA(ushort CodePage, const(char)* lpCurrentChar, uint dwFlags);
PSTR CharPrevExA(ushort CodePage, const(char)* lpStart, const(char)* lpCurrentChar, uint dwFlags);
...
WAIT_EVENT MsgWaitForMultipleObjects(uint nCount, const(HANDLE)* pHandles, BOOL fWaitAll, uint dwMilliseconds, QUEUE_STATUS_FLAGS dwWakeMask);
WAIT_EVENT MsgWaitForMultipleObjectsEx(uint nCount, const(HANDLE)* pHandles, uint dwMilliseconds, QUEUE_STATUS_FLAGS dwWakeMask, MSG_WAIT_FOR_MULTIPLE_OBJECTS_EX_FLAGS dwFlags);
ulong SetTimer(HWND hWnd, ulong nIDEvent, uint uElapse, TIMERPROC lpTimerFunc);
ulong SetCoalescableTimer(HWND hWnd, ulong nIDEvent, uint uElapse, TIMERPROC lpTimerFunc, uint uToleranceDelay);
BOOL KillTimer(HWND hWnd, ulong uIDEvent);
...
BOOL InsertMenuA(HMENU hMenu, uint uPosition, MENU_ITEM_FLAGS uFlags, ulong uIDNewItem, const(char)* lpNewItem);
BOOL InsertMenuW(HMENU hMenu, uint uPosition, MENU_ITEM_FLAGS uFlags, ulong uIDNewItem, const(wchar)* lpNewItem);
BOOL AppendMenuA(HMENU hMenu, MENU_ITEM_FLAGS uFlags, ulong uIDNewItem, const(char)* lpNewItem);
BOOL AppendMenuW(HMENU hMenu, MENU_ITEM_FLAGS uFlags, ulong uIDNewItem, const(wchar)* lpNewItem);
BOOL ModifyMenuA(HMENU hMnu, uint uPosition, MENU_ITEM_FLAGS uFlags, ulong uIDNewItem, const(char)* lpNewItem);
BOOL ModifyMenuW(HMENU hMnu, uint uPosition, MENU_ITEM_FLAGS uFlags, ulong uIDNewItem, const(wchar)* lpNewItem);
...
BOOL SetMenuItemBitmaps(HMENU hMenu, uint uPosition, MENU_ITEM_FLAGS uFlags, HBITMAP hBitmapUnchecked, HBITMAP hBitmapChecked);
int GetMenuCheckMarkDimensions();
BOOL TrackPopupMenu(HMENU hMenu, TRACK_POPUP_MENU_FLAGS uFlags, int x, int y, int nReserved, HWND hWnd, const(RECT)* prcRect);
...
uint DragObject(HWND hwndParent, HWND hwndFrom, uint fmt, ulong data, HCURSOR hcur);
...
int ScrollWindowEx(HWND hWnd, int dx, int dy, const(RECT)* prcScroll, const(RECT)* prcClip, HRGN hrgnUpdate, RECT* prcUpdate, SCROLL_WINDOW_FLAGS flags);
...
MESSAGEBOX_RESULT MessageBoxA(HWND hWnd, const(char)* lpText, const(char)* lpCaption, MESSAGEBOX_STYLE uType);
MESSAGEBOX_RESULT MessageBoxW(HWND hWnd, const(wchar)* lpText, const(wchar)* lpCaption, MESSAGEBOX_STYLE uType);
MESSAGEBOX_RESULT MessageBoxExA(HWND hWnd, const(char)* lpText, const(char)* lpCaption, MESSAGEBOX_STYLE uType, ushort wLanguageId);
MESSAGEBOX_RESULT MessageBoxExW(HWND hWnd, const(wchar)* lpText, const(wchar)* lpCaption, MESSAGEBOX_STYLE uType, ushort wLanguageId);
...
uint PrivateExtractIconsA(const(char)* szFileName, int nIconIndex, int cxIcon, int cyIcon, HICON* phicon, uint* piconid, uint nIcons, uint flags);
uint PrivateExtractIconsW(const(wchar)* szFileName, int nIconIndex, int cxIcon, int cyIcon, HICON* phicon, uint* piconid, uint nIcons, uint flags);
...
HICON CreateIconFromResourceEx(ubyte* presbits, uint dwResSize, BOOL fIcon, uint dwVer, int cxDesired, int cyDesired, IMAGE_FLAGS Flags);
HANDLE LoadImageA(HINSTANCE hInst, const(char)* name, GDI_IMAGE_TYPE type, int cx, int cy, IMAGE_FLAGS fuLoad);
HANDLE LoadImageW(HINSTANCE hInst, const(wchar)* name, GDI_IMAGE_TYPE type, int cx, int cy, IMAGE_FLAGS fuLoad);
HANDLE CopyImage(HANDLE h, GDI_IMAGE_TYPE type, int cx, int cy, IMAGE_FLAGS flags);
BOOL DrawIconEx(HDC hdc, int xLeft, int yTop, HICON hIcon, int cxWidth, int cyWidth, uint istepIfAniCur, HBRUSH hbrFlickerFreeDraw, DI_FLAGS diFlags);
...
HWND CreateMDIWindowA(const(char)* lpClassName, const(char)* lpWindowName, WINDOW_STYLE dwStyle, int X, int Y, int nWidth, int nHeight, HWND hWndParent, HINSTANCE hInstance, LPARAM lParam);
HWND CreateMDIWindowW(const(wchar)* lpClassName, const(wchar)* lpWindowName, WINDOW_STYLE dwStyle, int X, int Y, int nWidth, int nHeight, HWND hWndParent, HINSTANCE hInstance, LPARAM lParam);
ushort TileWindows(HWND hwndParent, TILE_WINDOWS_HOW wHow, const(RECT)* lpRect, uint cKids, const(HWND)* lpKids);
ushort CascadeWindows(HWND hwndParent, CASCADE_WINDOWS_HOW wHow, const(RECT)* lpRect, uint cKids, const(HWND)* lpKids);
BOOL SystemParametersInfoA(SYSTEM_PARAMETERS_INFO_ACTION uiAction, uint uiParam, void* pvParam, SYSTEM_PARAMETERS_INFO_UPDATE_FLAGS fWinIni);
BOOL SystemParametersInfoW(SYSTEM_PARAMETERS_INFO_ACTION uiAction, uint uiParam, void* pvParam, SYSTEM_PARAMETERS_INFO_UPDATE_FLAGS fWinIni);
BOOL SoundSentry();
...
BOOL ChangeWindowMessageFilterEx(HWND hwnd, uint message, WINDOW_MESSAGE_FILTER_ACTION action, CHANGEFILTERSTRUCT* pChangeFilterStruct);
BOOL SetAdditionalForegroundBoostProcesses(HWND topLevelWindow, uint processHandleCount, HANDLE* processHandleArray);
...
HRESULT IndexFilePath(void* resourceIndexer, const(wchar)* filePath, PWSTR* ppResourceUri, uint* pQualifierCount, IndexedResourceQualifier** ppQualifiers);
...
HRESULT MrmCreateResourceIndexerFromPreviousSchemaData(const(wchar)* projectRoot, MrmPlatformVersion platformVersion, const(wchar)* defaultQualifiers, ubyte* schemaXmlData, uint schemaXmlSize, MrmResourceIndexerHandle* indexer);
HRESULT MrmCreateResourceIndexerFromPreviousPriData(const(wchar)* projectRoot, MrmPlatformVersion platformVersion, const(wchar)* defaultQualifiers, ubyte* priData, uint priSize, MrmResourceIndexerHandle* indexer);
HRESULT MrmCreateResourceIndexerWithFlags(const(wchar)* packageFamilyName, const(wchar)* projectRoot, MrmPlatformVersion platformVersion, const(wchar)* defaultQualifiers, MrmIndexerFlags flags, MrmResourceIndexerHandle* indexer);
...
HRESULT MrmIndexEmbeddedData(MrmResourceIndexerHandle indexer, const(wchar)* resourceUri, const(ubyte)* embeddedData, uint embeddedDataSize, const(wchar)* qualifiers);
...
HRESULT MrmCreateResourceFileWithChecksum(MrmResourceIndexerHandle indexer, MrmPackagingMode packagingMode, MrmPackagingOptions packagingOptions, uint checksum, const(wchar)* outputDirectory);
...
HRESULT MrmPeekResourceIndexerMessages(MrmResourceIndexerHandle handle, MrmResourceIndexerMessage** messages, uint* numMsgs);
...
HRESULT MrmDumpPriDataInMemory(ubyte* inputPriData, uint inputPriSize, ubyte* schemaPriData, uint schemaPriSize, MrmDumpType dumpType, ubyte** outputXmlData, uint* outputXmlSize);
...
HRESULT MrmGetPriFileContentChecksum(const(wchar)* priFile, uint* checksum);
...
enum HWND_BROADCAST = 0x0000ffff;
...
enum WINSTA_ALL_ACCESS = 0x0000037f;
enum WVR_REDRAW = 0x00000300;
enum RT_GROUP_CURSOR = 0x000c;
enum RT_GROUP_ICON = 0x000e;
enum RT_MANIFEST = 0x0018;
enum IDC_STATIC = 0xffffffffffffffff;
alias DI_FLAGS = uint;
enum : uint
{
    DI_MASK        = 0x00000001,
...
alias DLGPROC = long function(HWND param0, uint param1, WPARAM param2, LPARAM param3);
alias TIMERPROC = void function(HWND param0, uint param1, ulong param2, uint param3);
...
alias SENDASYNCPROC = void function(HWND param0, uint param1, ulong param2, LRESULT param3);
...
alias PROPENUMPROCEXA = BOOL function(HWND param0, PSTR param1, HANDLE param2, ulong param3);
alias PROPENUMPROCEXW = BOOL function(HWND param0, PWSTR param1, HANDLE param2, ulong param3);
...
alias PREGISTERCLASSNAMEW = BOOLEAN function(const(wchar)* param0);
