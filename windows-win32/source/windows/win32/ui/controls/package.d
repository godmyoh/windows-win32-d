...
long PropertySheetA(PROPSHEETHEADERA_V2* param0);
long PropertySheetW(PROPSHEETHEADERW_V2* param0);
...
BOOL ImageList_DrawEx(HIMAGELIST himl, int i, HDC hdcDst, int x, int y, int dx, int dy, COLORREF rgbBk, COLORREF rgbFg, IMAGE_LIST_DRAW_STYLE fStyle);
...
HIMAGELIST ImageList_LoadImageA(HINSTANCE hi, const(char)* lpbmp, int cx, int cGrow, COLORREF crMask, uint uType, IMAGE_FLAGS uFlags);
HIMAGELIST ImageList_LoadImageW(HINSTANCE hi, const(wchar)* lpbmp, int cx, int cGrow, COLORREF crMask, uint uType, IMAGE_FLAGS uFlags);
...
HWND CreateToolbarEx(HWND hwnd, uint ws, uint wID, int nBitmaps, HINSTANCE hBMInst, ulong wBMID, TBBUTTON* lpButtons, int iNumButtons, int dxButton, int dyButton, int dxBitmap, int dyBitmap, uint uStructSize);
HBITMAP CreateMappedBitmap(HINSTANCE hInstance, long idBitmap, uint wFlags, COLORMAP* lpColorMap, int iNumMaps);
...
BOOL ShowHideMenuCtl(HWND hWnd, ulong uFlags, int* lpInfo);
...
HRESULT TaskDialog(HWND hwndOwner, HINSTANCE hInstance, const(wchar)* pszWindowTitle, const(wchar)* pszMainInstruction, const(wchar)* pszContent, TASKDIALOG_COMMON_BUTTON_FLAGS dwCommonButtons, const(wchar)* pszIcon, int* pnButton);
...
void* DPA_GetPtr(HDPA hdpa, long i);
...
BOOL DPA_Merge(HDPA hdpaDest, HDPA hdpaSrc, uint dwFlags, PFNDACOMPARE pfnCompare, PFNDPAMERGE pfnMerge, LPARAM lParam);
int DPA_Search(HDPA hdpa, void* pFind, int iStart, PFNDACOMPARE pfnCompare, LPARAM lParam, uint options);
BOOL Str_SetPtrW(PWSTR* ppsz, const(wchar)* psz);
...
BOOL FlatSB_SetScrollProp(HWND param0, uint index, long newValue, BOOL param3);
BOOL InitializeFlatSB(HWND param0);
HRESULT UninitializeFlatSB(HWND param0);
HRESULT LoadIconMetric(HINSTANCE hinst, const(wchar)* pszName, _LI_METRIC lims, HICON* phico);
HRESULT LoadIconWithScaleDown(HINSTANCE hinst, const(wchar)* pszName, int cx, int cy, HICON* phico);
int DrawShadowText(HDC hdc, const(wchar)* pszText, uint cch, RECT* prc, uint dwFlags, COLORREF crText, COLORREF crShadow, int ixOffset, int iyOffset);
...
HRESULT GetThemeAnimationProperty(HTHEME hTheme, int iStoryboardId, int iTargetId, TA_PROPERTY eProperty, void* pvProperty, uint cbSize, uint* pcbSizeOut);
HRESULT GetThemeAnimationTransform(HTHEME hTheme, int iStoryboardId, int iTargetId, uint dwTransformIndex, TA_TRANSFORM* pTransform, uint cbSize, uint* pcbSizeOut);
HRESULT GetThemeTimingFunction(HTHEME hTheme, int iTimingFunctionId, TA_TIMINGFUNCTION* pTimingFunction, uint cbSize, uint* pcbSizeOut);
HTHEME OpenThemeData(HWND hwnd, const(wchar)* pszClassList);
HTHEME OpenThemeDataEx(HWND hwnd, const(wchar)* pszClassList, OPEN_THEME_DATA_FLAGS dwFlags);
HRESULT CloseThemeData(HTHEME hTheme);
HRESULT DrawThemeBackground(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, RECT* pRect, RECT* pClipRect);
HRESULT DrawThemeBackgroundEx(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, RECT* pRect, const(DTBGOPTS)* pOptions);
HRESULT DrawThemeText(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, const(wchar)* pszText, int cchText, DRAW_TEXT_FORMAT dwTextFlags, uint dwTextFlags2, RECT* pRect);
HRESULT GetThemeBackgroundContentRect(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, RECT* pBoundingRect, RECT* pContentRect);
HRESULT GetThemeBackgroundExtent(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, RECT* pContentRect, RECT* pExtentRect);
...
HRESULT GetThemeTextExtent(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, const(wchar)* pszText, int cchCharCount, DRAW_TEXT_FORMAT dwTextFlags, RECT* pBoundingRect, RECT* pExtentRect);
HRESULT GetThemeTextMetrics(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, TEXTMETRICW* ptm);
HRESULT HitTestThemeBackground(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, HIT_TEST_BACKGROUND_OPTIONS dwOptions, RECT* pRect, HRGN hrgn, POINT ptTest, ushort* pwHitTestCode);
...
HRESULT GetThemeFilename(HTHEME hTheme, int iPartId, int iStateId, int iPropId, PWSTR pszThemeFileName, int cchMaxBuffChars);
...
HRESULT GetThemeSysString(HTHEME hTheme, int iStringId, PWSTR pszStringBuff, int cchMaxStringChars);
...
HRESULT GetCurrentThemeName(PWSTR pszThemeFileName, int cchMaxNameChars, PWSTR pszColorBuff, int cchMaxColorChars, PWSTR pszSizeBuff, int cchMaxSizeChars);
HRESULT GetThemeDocumentationProperty(const(wchar)* pszThemeName, const(wchar)* pszPropertyName, PWSTR pszValueBuff, int cchMaxValChars);
HRESULT DrawThemeParentBackground(HWND hwnd, HDC hdc, const(RECT)* prc);
HRESULT EnableTheming(BOOL fEnable);
HRESULT DrawThemeParentBackgroundEx(HWND hwnd, HDC hdc, DRAW_THEME_PARENT_BACKGROUND_FLAGS dwFlags, const(RECT)* prc);
HRESULT SetWindowThemeAttribute(HWND hwnd, WINDOWTHEMEATTRIBUTETYPE eAttribute, void* pvAttribute, uint cbAttribute);
HRESULT DrawThemeTextEx(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, const(wchar)* pszText, int cchText, DRAW_TEXT_FORMAT dwTextFlags, RECT* pRect, const(DTTOPTS)* pOptions);
...
HRESULT GetThemeStream(HTHEME hTheme, int iPartId, int iStateId, int iPropId, void** ppvStream, uint* pcbStream, HINSTANCE hInst);
HRESULT BufferedPaintInit();
HRESULT BufferedPaintUnInit();
long BeginBufferedPaint(HDC hdcTarget, const(RECT)* prcTarget, BP_BUFFERFORMAT dwFormat, BP_PAINTPARAMS* pPaintParams, HDC* phdc);
HRESULT EndBufferedPaint(long hBufferedPaint, BOOL fUpdateTarget);
HRESULT GetBufferedPaintTargetRect(long hBufferedPaint, RECT* prc);
HDC GetBufferedPaintTargetDC(long hBufferedPaint);
HDC GetBufferedPaintDC(long hBufferedPaint);
HRESULT GetBufferedPaintBits(long hBufferedPaint, RGBQUAD** ppbBuffer, int* pcxRow);
HRESULT BufferedPaintClear(long hBufferedPaint, const(RECT)* prc);
HRESULT BufferedPaintSetAlpha(long hBufferedPaint, const(RECT)* prc, ubyte alpha);
HRESULT BufferedPaintStopAllAnimations(HWND hwnd);
long BeginBufferedAnimation(HWND hwnd, HDC hdcTarget, const(RECT)* prcTarget, BP_BUFFERFORMAT dwFormat, BP_PAINTPARAMS* pPaintParams, BP_ANIMATIONPARAMS* pAnimationParams, HDC* phdcFrom, HDC* phdcTo);
HRESULT EndBufferedAnimation(long hbpAnimation, BOOL fUpdateTarget);
BOOL BufferedPaintRenderAnimation(HWND hwnd, HDC hdcTarget);
BOOL IsCompositionActive();
HRESULT GetThemeTransitionDuration(HTHEME hTheme, int iPartId, int iStateIdFrom, int iStateIdTo, int iPropId, uint* pdwDuration);
...
HSYNTHETICPOINTERDEVICE CreateSyntheticPointerDevice(POINTER_INPUT_TYPE pointerType, uint maxCount, POINTER_FEEDBACK_MODE mode);
...
BOOL EvaluateProximityToPolygon(uint numVertices, const(POINT)* controlPolygon, const(TOUCH_HIT_TESTING_INPUT)* pHitTestingInput, TOUCH_HIT_TESTING_PROXIMITY_EVALUATION* pProximityEval);
...
BOOL SetWindowFeedbackSetting(HWND hwnd, FEEDBACK_TYPE feedback, uint dwFlags, uint size, const(void)* configuration);
...
int DlgDirListComboBoxA(HWND hDlg, PSTR lpPathSpec, int nIDComboBox, int nIDStaticPath, DLG_DIR_LIST_FILE_TYPE uFiletype);
int DlgDirListComboBoxW(HWND hDlg, PWSTR lpPathSpec, int nIDComboBox, int nIDStaticPath, DLG_DIR_LIST_FILE_TYPE uFiletype);
...
alias PFTASKDIALOGCALLBACK = HRESULT function(HWND hwnd, uint msg, WPARAM wParam, LPARAM lParam, long lpRefData);
