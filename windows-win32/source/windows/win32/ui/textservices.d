...
import windows.win32.system.com : FORMATETC, IDataObject, IEnumGUID, IEnumString, IEnumUnknown, IStream, IUnknown;
...
enum TF_DEFAULT_SELECTION = 0xffffffff;
enum TS_DEFAULT_SELECTION = 0xffffffff;
...
interface ITextStoreACP : IUnknown
{
    ...
    HRESULT GetText(int acpStart, int acpEnd, PWSTR pchPlain, uint cchPlainReq, uint* pcchPlainRet, TS_RUNINFO* prgRunInfo, uint cRunInfoReq, uint* pcRunInfoRet, int* pacpNext);
    ...
    HRESULT RequestAttrsTransitioningAtPosition(int acpPos, uint cFilterAttrs, const(GUID)* paFilterAttrs, uint dwFlags);
    HRESULT FindNextAttrTransition(int acpStart, int acpHalt, uint cFilterAttrs, const(GUID)* paFilterAttrs, uint dwFlags, int* pacpNext, BOOL* pfFound, int* plFoundOffset);
...
interface ITextStoreAnchor : IUnknown
{
    ...
    HRESULT GetText(uint dwFlags, IAnchor paStart, IAnchor paEnd, PWSTR pchText, uint cchReq, uint* pcch, BOOL fUpdateAnchor);
    ...
    HRESULT RequestAttrsTransitioningAtPosition(IAnchor paPos, uint cFilterAttrs, const(GUID)* paFilterAttrs, uint dwFlags);
    HRESULT FindNextAttrTransition(IAnchor paStart, IAnchor paHalt, uint cFilterAttrs, const(GUID)* paFilterAttrs, uint dwFlags, BOOL* pfFound, int* plFoundOffset);
    HRESULT RetrieveRequestedAttrs(uint ulCount, TS_ATTRVAL* paAttrVals, uint* pcFetched);
...
interface ITfLangBarItemMgr : IUnknown
{
    ...
    HRESULT GetItems(uint ulCount, ITfLangBarItem* ppItem, TF_LANGBARITEMINFO* pInfo, uint* pdwStatus, uint* pcFetched);
...
{
    HRESULT RequestEditSession(uint tid, ITfEditSession pes, TF_CONTEXT_EDIT_CONTEXT_FLAGS dwFlags, HRESULT* phrSession);
    ...
    HRESULT TrackProperties(const(GUID)** prgProp, uint cProp, const(GUID)** prgAppProp, uint cAppProp, ITfReadOnlyProperty* ppProperty);
...
{
    HRESULT InsertTextAtSelection(uint ec, INSERT_TEXT_AT_SELECTION_FLAGS dwFlags, const(wchar)* pchText, int cch, ITfRange* ppRange);
...
interface ITfEditRecord : IUnknown
{
    ...
    HRESULT GetTextAndPropertyUpdates(GET_TEXT_AND_PROPERTY_UPDATES_FLAGS dwFlags, const(GUID)** prgProperties, uint cProperties, IEnumTfRanges* ppEnum);
}
...
{
    HRESULT IsStoreSerializable(const(GUID)* guidProp, ITfRange pRange, ITfPropertyStore pPropStore, BOOL* pfSerializable);
...
{
    HRESULT ActivateProfile(uint dwProfileType, ushort langid, const(GUID)* clsid, const(GUID)* guidProfile, HKL hkl, uint dwFlags);
    HRESULT DeactivateProfile(uint dwProfileType, ushort langid, const(GUID)* clsid, const(GUID)* guidProfile, HKL hkl, uint dwFlags);
    HRESULT GetProfile(uint dwProfileType, ushort langid, const(GUID)* clsid, const(GUID)* guidProfile, HKL hkl, TF_INPUTPROCESSORPROFILE* pProfile);
    ...
    HRESULT RegisterProfile(const(GUID)* rclsid, ushort langid, const(GUID)* guidProfile, const(wchar)* pchDesc, uint cchDesc, const(wchar)* pchIconFile, uint cchFile, uint uIconIndex, HKL hklsubstitute, uint dwPreferredLayout, BOOL bEnabledByDefault, uint dwFlags);
...
{
    HRESULT OnActivated(uint dwProfileType, ushort langid, const(GUID)* clsid, const(GUID)* catid, const(GUID)* guidProfile, HKL hkl, uint dwFlags);
}
...
{
    HRESULT PeekMessageA(MSG* pMsg, HWND hwnd, uint wMsgFilterMin, uint wMsgFilterMax, uint wRemoveMsg, BOOL* pfResult);
    HRESULT GetMessageA(MSG* pMsg, HWND hwnd, uint wMsgFilterMin, uint wMsgFilterMax, BOOL* pfResult);
    HRESULT PeekMessageW(MSG* pMsg, HWND hwnd, uint wMsgFilterMin, uint wMsgFilterMax, uint wRemoveMsg, BOOL* pfResult);
...
{
    HRESULT OnTransitoryExtensionUpdated(ITfContext pic, uint ecReadOnly, ITfRange pResultRange, ITfRange pCompositionRange, BOOL* pfDeleteResultRange);
}
